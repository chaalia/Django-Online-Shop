...
from .five import items, monotonic, python_2_unicode_compatible, reraise, values
...
class Scheduler(object):
    ...
    def should_sync(self):
        return (
            not self._last_sync or (monotonic() - self._last_sync) > self.sync_every
    ...
    def update_from_dict(self, dict_):
        self.schedule.update(
            {name: self._maybe_entry(name, entry) for name, entry in items(dict_)}
        )
...
class PersistentScheduler(Scheduler):
    ...
    def setup_schedule(self):
        ...
        self._store.update(
            {str("__version__"): __version__, str("tz"): tz, str("utc_enabled"): utc}
        )
        self.sync()
        debug(
...
class Service(object):
    ...
    def __reduce__(self):
        return (
            self.__class__,
            (self.max_interval, self.schedule_filename, self.scheduler_cls, self.app),
        )
