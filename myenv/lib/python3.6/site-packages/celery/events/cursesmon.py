...
class CursesMonitor(object):  # pragma: no cover
    ...
    greet = "celery events {}".format(VERSION_BANNER)
    info_str = "Info: "
    ...
    def format_row(self, uuid, task, worker, timestamp, state):
        ...
        row = "{} {} {} {} {} ".format(uuid, worker, task, timestamp, state)
    ...
    def selection_result(self):
        ...
        return self.alert(
            alert_callback, "Task Result for {0.selected_task}".format(self)
        )
        ...
        with self.lock:
            ...
            if self.selected_task:
                ...
                try:
                    ...
                    info = " ".join(
                        "{}={}".format(key, value) for key, value in items(info)
                    )
                    detail = "... -> key i"
                infowin = abbr(
            ...
            win.addstr(
                ...
                STATUS_SCREEN.format(
                    s=self.state,
                    w_alive=len([w for w in values(self.state.workers) if w.alive]),
    ...
    @property
    def workers(self):
        return [hostname for hostname, w in items(self.state.workers) if w.alive]
    ...
    while 1:
        ...
        with app.connection_for_read() as conn:
            try:
                ...
                print("Connection lost: {!r}".format(exc), file=sys.stderr)
