...
from .five import items, monotonic, python_2_unicode_compatible, range, string_t
...
class AsyncResult(ResultBase):
    ...
    def __init__(
        ...
        if id is None:
            raise ValueError("AsyncResult requires valid id, not {}".format(type(id)))
    ...
    def __copy__(self):
        return self.__class__(self.id, self.backend, None, self.app, self.parent)
...
class ResultSet(ResultBase):
    ...
    def iterate(self, timeout=None, propagate=True, interval=0.5):
        ...
        while results:
            ...
            for task_id, result in items(results):
                if result.ready():
                    yield result.get(
                        timeout=timeout and timeout - elapsed, propagate=propagate
                    )
                    removed.add(task_id)
                else:
    ...
    def __repr__(self):
        return "<{}: [{}]>".format(
            type(self).__name__, ", ".join(r.id for r in self.results)
        )
...
class GroupResult(ResultSet):
    ...
    def __repr__(self):
        return "<{}: {} [{}]>".format(
            type(self).__name__, self.id, ", ".join(r.id for r in self.results)
        )
