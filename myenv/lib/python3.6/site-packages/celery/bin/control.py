...
class _RemoteControl(Command):
    ...
    def get_command_info(
        ...
        return text.join(
            [
                "|"
                + text.indent(
                    "{}{} {}".format(prefix, color(command), meta.signature or ""),
                    indent,
                ),
                help,
            ]
    ...
    @classmethod
    def list_commands(cls, indent=0, prefix="", color=None, help=False, app=None):
        choices = cls._choices_by_group(app)
        color = color if color else lambda x: x
        prefix = prefix + " " if prefix else ""
        return "\n".join(
            cls.get_command_info(
                c, indent, prefix, color, help, app=app, choices=choices
            )
    ...
    def run(self, *args, **kwargs):
        if not args:
            raise self.UsageError("Missing {0.name} method.  See --help".format(self))
    ...
    def do_call_method(
        self, args, timeout=None, destination=None, json=False, **kwargs
    ):
        method = args[0]
        if method == "help":
        ...
        try:
            ...
            raise self.UsageError("Unknown {0.name} method {1}".format(self, method))
        ...
        if not replies:
            raise self.Error(
                "No nodes replied within time constraint.", status=EX_UNAVAILABLE
            )
        if json:
    ...
    def compile_arguments(self, meta, method, args):
        ...
        kw = {}
        if meta.args:
            kw.update({k: v for k, v in self._consume_args(meta, method, args)})
        if meta.variadic:
            kw.update({meta.variadic: args})
        if not kw and args:
            raise self.Error(
                "Command {!r} takes no arguments.".format(method), status=EX_USAGE
            )
        return kw or {}
    ...
    def epilog(self):
        return "\n".join(
            ["[Commands]", self.list_commands(indent=4, help=True, app=self.app)]
        )
...
class inspect(_RemoteControl):
    ...
    def call(self, method, arguments, **options):
        return self.app.control.inspect(**options)._request(method, **arguments)
