...
class MultiTool(TermLogger):
    ...
    def call_command(self, command, argv):
        try:
            ...
            return self.error("Invalid command: {}".format(command))
    ...
    def on_node_restart(self, node):
        self.note(
            self.colored.blue("> Restarting node {0.name}: ".format(node)),
            newline=False,
        )
    ...
    def on_child_spawn(self, node, argstr, env):
        self.info("  {}".format(argstr))

    def on_child_signalled(self, node, signum):
        self.note("* Child was terminated by signal {}".format(signum))

    def on_child_failure(self, node, retcode):
        self.note("* Child terminated with exit code {}".format(retcode))
