...
class shell(Command):  # pragma: no cover
    ...
    def add_arguments(self, parser):
        ...
        group.add_argument(
            "--ipython", "-I", action="store_true", help="force iPython.", default=False
        )
        group.add_argument(
            "--bpython", "-B", action="store_true", help="force bpython.", default=False
        )
        group.add_argument(
            "--python",
            action="store_true",
        ...
        group.add_argument(
            "--eventlet", action="store_true", default=False, help="use eventlet."
        )
        group.add_argument(
            "--gevent", action="store_true", default=False, help="use gevent."
        )

    def run(self, *args, **kwargs):
        if args:
            raise self.UsageError(
                "shell command does not take arguments: {}".format(args)
            )
    ...
    def invoke_ipython_shell(self):
        for ip in (
            self._ipython,
            self._ipython_pre_10,
            self._ipython_terminal,
            self._ipython_010,
            self._no_ipython,
        ):
            try:
