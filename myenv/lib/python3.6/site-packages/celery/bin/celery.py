...
class CeleryCommand(Command):
    ...
    def on_usage_error(self, exc, command=None):
        ...
        self.error(self.colored.magenta("Error: {}".format(exc)))
        self.error(
            """Please try '{}'""".format(helps.format(self=self, command=command))
        )
    ...
    @classmethod
    def get_command_info(cls, command, indent=0, color=None, colored=None, app=None):
        ...
        return text.join(
            [
                " ",
                "|" + text.indent("{} --help".format(cmd), indent),
                obj.list_commands(
                    indent, "celery {}".format(command), colored, app=app
                ),
            ]
        )
    ...
    def load_extension_commands(self):
        names = Extensions(self.ext_fmt.format(self=self), self.register_command).load()
        if names:
