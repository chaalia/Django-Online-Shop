...
class detached_celeryd(object):
    ...
    description = (
        "Detaches Celery worker nodes.  See `celery worker --help` "
    ...
    def create_parser(self, prog_name):
        parser = argparse.ArgumentParser(
            prog=prog_name, usage=self.usage, description=self.description
        )
    ...
    def parse_options(self, prog_name, argv):
        ...
        if options.logfile:
            leftovers.append("--logfile={}".format(options.logfile))
        if options.pidfile:
            leftovers.append("--pidfile={}".format(options.pidfile))
        if options.hostname:
            leftovers.append("--hostname={}".format(options.hostname))
