...
__all__ = ("Error", "UsageError", "Extensions", "Command", "Option", "daemon_options")
...
class Command(object):
    ...
    def on_error(self, exc):
        ...
        self.error(self.colored.red("Error: {}".format(exc)))
    ...
    def add_preload_arguments(self, parser):
        ...
        group.add_argument("--no-color", "-C", action="store_true", default=None)
    ...
    def ask(self, q, choices, default=None):
        ...
        if default is not None:
            schoices = [c.upper() if c == default else c.lower() for c in choices]
    ...
    def setup_app_from_commandline(self, argv):
        ...
        if workdir:
            os.chdir(workdir)
        app = preload_options.get("app") or os.environ.get("CELERY_APP") or self.app
        preload_loader = preload_options.get("loader")
        if preload_loader:
            # Default app takes loader from this env (Issue #1066).
            os.environ["CELERY_LOADER"] = preload_loader
        loader = (preload_loader, os.environ.get("CELERY_LOADER") or "default")
    ...
    def pretty_list(self, n):
        ...
        if not n:
            return "- empty -"
        return "\n".join(str(c.reset(c.white("*"), " {}".format(item))) for item in n)
