...
class worker(Command):
    ...
    def run(
        ...
        **kwargs
        ...
        # as possible.
        pool_cls = concurrency.get_implementation(pool_cls) or self.app.conf.worker_pool
        ...
        if loglevel:
            try:
                ...
                self.die(
                    "Unknown level {!r}.  Please use one of {}.".format(
                        loglevel,
    ...
    def add_arguments(self, parser):
        ...
        wopts.add_argument("-D", "--detach", action="store_true", default=False)
        wopts.add_argument("-S", "--statedb", default=conf.worker_state_db)
        wopts.add_argument("-l", "--loglevel", default="WARN")
        wopts.add_argument("-O", dest="optimization")
        wopts.add_argument(
            "--prefetch-multiplier", type=int, default=conf.worker_prefetch_multiplier
        )

        topts = parser.add_argument_group("Pool Options")
        topts.add_argument(
            "-c", "--concurrency", default=conf.worker_concurrency, type=int
        )
        topts.add_argument("-P", "--pool", default=conf.worker_pool)
        topts.add_argument(
            "-E",
            "--task-events",
            "--events",
        ...
        topts.add_argument("--time-limit", type=float, default=conf.task_time_limit)
        topts.add_argument(
            "--soft-time-limit", type=float, default=conf.task_soft_time_limit
        )
        topts.add_argument(
            "--max-tasks-per-child",
            "--maxtasksperchild",
        ...
        qopts.add_argument("--purge", "--discard", action="store_true", default=False)
        ...
        fopts.add_argument("--without-gossip", action="store_true", default=False)
        fopts.add_argument("--without-mingle", action="store_true", default=False)
        fopts.add_argument("--without-heartbeat", action="store_true", default=False)
