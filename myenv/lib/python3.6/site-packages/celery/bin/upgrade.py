...
class upgrade(Command):
    ...
    def run(self, *args, **kwargs):
        ...
        if command not in self.choices:
            raise self.UsageError("unknown upgrade type: {}".format(command))
    ...
    def settings(
        ...
        **kwargs
        ...
        print("processing {}...".format(filename), file=self.stderr)
        # gives list of tuples: ``(did_change, line_contents)``
        new_lines = [self._to_new_key(line, keyfilter) for line in lines]
        if any(n[0] for n in new_lines):  # did have changes
            ...
            with codecs.open(filename, "w", "utf-8") as write_fh:
                for _, line in new_lines:
                    write_fh.write(line)
            print("Changes to your setting have been made!", file=self.stdout)
        else:
            print("Does not seem to require any changes :-)", file=self.stdout)
    ...
    def _backup(self, filename, suffix=".orig"):
        ...
        print("writing backup to {}...".format(backup_filename), file=self.stderr)
        with codecs.open(filename, "r", "utf-8") as read_fh:
