...
class Signature(dict):
    ...
    def freeze(self, _id=None, group_id=None, chord=None, root_id=None, parent_id=None):
...
class _chain(Signature):
    ...
    def prepare_steps(
        ...
        while steps:
            ...
            if is_last_task:
                ...
                res = task.freeze(
                    last_task_id, root_id=root_id, group_id=group_id, chord=chord_body
                )
            else:
    ...
    def __repr__(self):
        if not self.tasks:
            return "<{}@{:#x}: empty>".format(type(self).__name__, id(self))
...
class _basemap(Signature):
    ...
    @classmethod
    def from_dict(cls, d, app=None):
        return _upgrade(d, cls(*cls._unpack_args(d["kwargs"]), app=app, **d["options"]))
...
class xmap(_basemap):
    ...
    def __repr__(self):
        task, it = self._unpack_args(self.kwargs)
        return "[{}(x) for x in {}]".format(task.task, truncate(repr(it), 100))
...
class chunks(Signature):
    ...
    def from_dict(cls, d, app=None):
        return _upgrade(
            d, chunks(*cls._unpack_args(d["kwargs"]), app=app, **d["options"])
        )
...
class group(Signature):
    ...
    @classmethod
    def from_dict(cls, d, app=None):
        return _upgrade(d, group(d["kwargs"]["tasks"], app=app, **d["options"]))
    ...
    def freeze(self, _id=None, group_id=None, chord=None, root_id=None, parent_id=None):
        ...
        results = list(
            self._freeze_unroll(new_tasks, group_id, chord, root_id, parent_id)
        )
    ...
    def _freeze_unroll(self, new_tasks, group_id, chord, root_id, parent_id):
        ...
        while stack:
            ...
            if isinstance(task, group):
                ...
                yield task.freeze(
                    group_id=group_id, chord=chord, root_id=root_id, parent_id=parent_id
                )

    def __repr__(self):
        if self.tasks:
            return remove_repeating_from_task(
                self.tasks[0]["task"], "group({0.tasks!r})".format(self)
            )
        return "group(<empty>)"
