...
from celery.five import UserDict, bytes_if_py2, python_2_unicode_compatible, values
...
from celery.utils.time import get_exponential_backoff_interval, timezone, to_utc
...
class Celery(object):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.on_after_configure = Signal(
            name="app.on_after_configure", providing_args={"source"}
        )
    ...
    def _task_from_fun(self, fun, name=None, base=None, bind=False, **options):
        ...
        if name not in self._tasks:
            ...
            retry_backoff_max = int(
                options.get(
                    "retry_backoff_max", getattr(task, "retry_backoff_max", 600)
                )
            )
    ...
    def autodiscover_tasks(self, packages=None, related_name="tasks", force=False):
        ...
        signals.import_modules.connect(
            starpromise(self._autodiscover_tasks, packages, related_name),
            weak=False,
            sender=self,
        )
    ...
    def producer_or_acquire(self, producer=None):
        ...
        """
        return FallbackContext(producer, self.producer_pool.acquire, block=True)
    ...
    def either(self, default_key, *defaults):
        ...
        """
        return first(
            None, [first(None, defaults), starpromise(self.conf.get, default_key)]
        )
    ...
    def __reduce_v1__(self):
        ...
        # between processes.
        return (_unpickle_app, (self.__class__, self.Pickler) + self.__reduce_args__())
