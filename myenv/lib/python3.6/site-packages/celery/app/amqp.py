...
class Queues(dict):
    ...
    def _set_ha_policy(self, args):
        ...
        if isinstance(policy, (list, tuple)):
            return args.update({"ha-mode": "nodes", "ha-params": list(policy)})
        args["ha-mode"] = policy
    ...
    def format(self, indent=0, indent_first=True):
        ...
        if not active:
            return ""
        info = [QUEUE_FORMAT.strip().format(q) for _, q in sorted(items(active))]
        if indent_first:
            return textindent("\n".join(info), indent)
        return info[0] + "\n" + textindent("\n".join(info[1:]), indent)
...
class AMQP(object):
    ...
    def _verify_seconds(self, s, what):
        if s < INT_MIN:
            raise ValueError("{} is out of range: {!r}".format(what, s))
        return s
            ...
            **kwargs
            ...
            if sent_event:
                ...
                if isinstance(exname, Exchange):
                    exname = exname.name
                sent_event.update(
                    {"queue": qname, "exchange": exname, "routing_key": routing_key}
                )
                evd.publish(
                    "task-sent",
                    sent_event,
