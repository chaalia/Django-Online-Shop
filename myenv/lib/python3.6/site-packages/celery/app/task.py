...
def _reprtask(task, fmt=None, flags=None):
    ...
    return fmt.format(
        task, flags=_strflags(flags), app=appstr(task._app) if task._app else None
    )
...
class Task(object):
    ...
    def apply(
        ...
        **options
        ...
        tb = None
        tracer = build_tracer(
            task.name, task, eager=True, propagate=throw, app=self._get_app()
        )
    ...
    def replace(self, sig):
        ...
        if isinstance(sig, group):
            sig |= (
                ...
                .s(index=0)
                .set(link=self.request.callbacks, link_error=self.request.errbacks)
            )
        ...
        sig.set(chord=chord, group_id=self.request.group, root_id=self.request.root_id)
