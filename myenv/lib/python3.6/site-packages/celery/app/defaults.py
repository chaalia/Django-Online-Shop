...
NAMESPACES = Namespace(
    ...
    couchbase=Namespace(
        __old__=old_ns("celery_couchbase"), backend_settings=Option(None, type="dict")
    ),
    arangodb=Namespace(
        __old__=old_ns("celery_arangodb"), backend_settings=Option(None, type="dict")
    ),
    mongodb=Namespace(
        __old__=old_ns("celery_mongodb"), backend_settings=Option(type="dict")
    ),
    ...
    result=Namespace(
        ...
        cache_max=Option(-1, type="int", old={"celery_max_cached_results"}),
        compression=Option(type="str"),
        exchange=Option("celeryresults"),
        exchange_type=Option("direct"),
        expires=Option(
            timedelta(days=1), type="float", old={"celery_task_result_expires"}
        ),
    ...
    riak=Namespace(__old__=old_ns("celery_riak"), backend_settings=Option(type="dict")),
    security=Namespace(
    ...
    database=Namespace(
        ...
        engine_options=Option(type="dict", old={"celery_result_engine_options"}),
    ...
    task=Namespace(
        ...
        publish_retry=Option(True, type="bool", old={"celery_task_publish_retry"}),
        ...
        soft_time_limit=Option(type="float", old={"celeryd_task_soft_time_limit"}),
        time_limit=Option(type="float", old={"celeryd_task_time_limit"}),
    ...
    worker=Namespace(
        ...
        redirect_stdouts=Option(True, type="bool", old={"celery_redirect_stdouts"}),
        redirect_stdouts_level=Option("WARNING", old={"celery_redirect_stdouts_level"}),
        send_task_events=Option(False, type="bool", old={"celery_send_events"}),
        state_db=Option(),
...
def find_deprecated_settings(source):  # pragma: no cover
    ...
    for name, opt in flatten(NAMESPACES):
        if (opt.deprecate_by or opt.remove_by) and getattr(source, name, None):
            deprecated.warn(
                description="The {!r} setting".format(name),
