...
HIDDEN_SETTINGS = re.compile(
    "API|TOKEN|KEY|SECRET|PASS|PROFANITIES_LIST|SIGNATURE|DATABASE", re.IGNORECASE
)
...
class Settings(ConfigurationView):
    ...
    def task_default_exchange(self):
        return self.first("task_default_exchange", "task_default_queue")

    @property
    def task_default_routing_key(self):
        return self.first("task_default_routing_key", "task_default_queue")
    ...
    def table(self, with_defaults=False, censored=True):
        ...
        return filt(
            {
                k: v
                for k, v in items(self if with_defaults else self.without_defaults())
...
_settings_info_t = namedtuple(
    "settings_info_t", ("defaults", "convert", "key_t", "mix_error")
)
...
def detect_settings(
    ...
    return Settings(
        preconf, [conf, defaults], (_old_key_to_new, _new_key_to_old), prefix=prefix
    )
