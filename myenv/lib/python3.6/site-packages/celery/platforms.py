        ...
        with ignore_errno("ENOENT"):
            with open(self.path, "r") as fh:
                ...
                if line.strip() == line:  # must contain '\n'
                    raise ValueError("Partial or invalid pidfile {0.path}".format(self))

                try:
                    return int(line.strip())
                except ValueError:
                    raise ValueError("pidfile {0.path} contents invalid.".format(self))
...
def parse_uid(uid):
    ...
    try:
        ...
        try:
            ...
            raise KeyError("User does not exist: {}".format(uid))
...
def parse_gid(gid):
    ...
    try:
        ...
        try:
            ...
            raise KeyError("Group does not exist: {}".format(gid))
...
class Signals(object):
    ...
    def signum(self, name):
        ...
        if isinstance(name, numbers.Integral):
            return name
        if not isinstance(name, string_t) or not name.isupper():
...
def set_process_title(progname, info=None):
    ...
    """
    proctitle = "[{}]".format(progname)
    proctitle = "{} {}".format(proctitle, info) if info else proctitle
    if _setproctitle:
...
if os.environ.get("NOSETPS"):  # pragma: no cover
    ...
    def set_mp_process_title(progname, info=None, hostname=None):  # noqa
        ...
        """
        if hostname:
            progname = "{}: {}".format(progname, hostname)
        name = current_process().name if current_process else "MainProcess"
        return set_process_title("{}:{}".format(progname, name), info=info)
    ...
    if not uid or not gid or not euid or not egid:
        if (
            ...
            if not C_FORCE_ROOT:
                try:
                    print(
                        ROOT_DISALLOWED.format(uid=uid, euid=euid, gid=gid, egid=egid),
                        file=sys.stderr,
                    )
                finally:
                    sys.stderr.flush()
        ...
        warnings.warn(
            RuntimeWarning(
                ROOT_DISCOURAGED.format(uid=uid, euid=euid, gid=gid, egid=egid)
            )
        )
