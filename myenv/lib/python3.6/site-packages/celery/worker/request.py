...
class Request(object):
        ...
        **opts
        ...
        if eta is not None:
            try:
                ...
                raise InvalidTaskError("invalid ETA value {!r}: {}".format(eta, exc))
        ...
        if expires is not None:
            try:
                ...
                raise InvalidTaskError(
                    "invalid expires value {!r}: {}".format(expires, exc)
                )
    ...
    def execute(self, loglevel=None, logfile=None):
        ...
        request.update(
            {"loglevel": loglevel, "logfile": logfile, "is_eager": False}, **embed or {}
        )
        retval = trace_task(
            self.task,
            self.id,
    ...
    def _announce_revoked(self, reason, terminated, signum, expired):
        ...
        self.task.backend.mark_as_revoked(
            self.id, reason, request=self._context, store_result=self.store_errors
        )
        self.acknowledge()
    ...
    def on_timeout(self, soft, timeout):
        ...
        if soft:
            ...
            self.task.backend.mark_as_failure(
                self.id, exc, request=self._context, store_result=self.store_errors
            )
    ...
    def on_failure(self, exc_info, send_failed_event=True, return_ok=False):
        ...
        if isinstance(exc_info.exception, MemoryError):
            raise MemoryError("Process got: {}".format(exc_info.exception))
    ...
    def __str__(self):
        ...
        return " ".join(
            [
                self.humaninfo(),
                " ETA:[{}]".format(self._eta) if self._eta else "",
                " expires:[{}]".format(self._expires) if self._expires else "",
            ]
        )
    ...
    def __repr__(self):
        """``repr(self)``."""
        return "<{}: {} {} {}>".format(
            type(self).__name__, self.humaninfo(), self._argsrepr, self._kwargsrepr
        )
