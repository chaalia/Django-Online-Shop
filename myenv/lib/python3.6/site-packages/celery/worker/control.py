...
def revoke(state, task_id, terminate=False, signal=None, **kwargs):
    ...
    return ok("tasks {} flagged as revoked".format(idstr))
...
def rate_limit(state, task_name, rate_limit, **kwargs):
    ...
    try:
        ...
        return nok("Invalid rate limit string: {!r}".format(exc))
...
def autoscale(state, max=None, min=None):
    ...
    if autoscaler:
        ...
        return ok("autoscale now max={} min={}".format(max_, min_))
...
def add_consumer(
    ...
    return ok("add consumer {}".format(queue))


@control_command(args=[("queue", text_t)], signature="<queue>")
def cancel_consumer(state, queue, **_):
    ...
    return ok("no longer consuming from {}".format(queue))
