...
def build_nodename(name, prefix, suffix):
    ...
    if "@" in name:
        ...
        shortname = "{}{}".format(prefix, name)
        nodename = host_format("{}@{}".format(shortname, hostname))
    return name, nodename, hostname


def build_expander(nodename, shortname, hostname):
    return partial(
        node_format, name=nodename, N=shortname, d=hostname, h=nodename, i="%i", I="%I"
    )
...
def _kwargs_to_command_line(kwargs):
    return {
        (
            "--{}".format(k.replace("_", "-")) if len(k) > 1 else "-{}".format(k)
        ): "{}".format(v)
...
class Node(object):
    ...
    def _waitexec(
        ...
        return self.handle_process_exit(
            pipe.wait(), on_signalled=on_signalled, on_failure=on_failure
        )
    ...
    def _update_ns_opts(self, p, names):
        ...
        for ns_name, ns_opts in list(items(p.namespaces)):
            if ns_name.isdigit():
                ...
                if ns_index < 0:
                    raise KeyError("Indexes start at 1 got: {!r}".format(ns_name))
                try:
                    p.namespaces[names[ns_index]].update(ns_opts)
                except IndexError:
                    raise KeyError("No node at index {!r}".format(ns_name))
...
class Cluster(UserList):
    ...
    def __repr__(self):
        return "<{name}({0}): {1}>".format(
            len(self), saferepr([n.name for n in self]), name=type(self).__name__
        )
