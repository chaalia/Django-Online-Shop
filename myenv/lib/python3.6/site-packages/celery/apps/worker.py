...
def safe_say(msg):
    print("\n{}".format(msg), file=sys.__stderr__)
...
class Worker(WorkController):
    ...
    def on_before_init(self, quiet=False, **kwargs):
        ...
        signals.celeryd_init.send(
            sender=self.hostname, instance=self, conf=self.app.conf, options=kwargs
        )
...
def _shutdown_handler(
    ...
    def _handle_request(*args):
        with in_sighandler():
            ...
            if current_process()._name == "MainProcess":
                if callback:
                    callback(worker)
                safe_say("worker: {} shutdown (MainProcess)".format(how))
    ...
    _handle_request.__name__ = str("worker_{}".format(how))
...
if not is_jython:  # pragma: no cover
    install_worker_int_handler = partial(
        _shutdown_handler, sig="SIGINT", callback=on_SIGINT, exitcode=EX_FAILURE
    )
else:  # pragma: no cover
