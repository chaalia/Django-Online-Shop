...
after_task_publish = Signal(
    name="after_task_publish", providing_args={"body", "exchange", "routing_key"}
)
task_received = Signal(name="task_received", providing_args={"request"})
task_prerun = Signal(
    name="task_prerun", providing_args={"task_id", "task", "args", "kwargs"}
)
task_postrun = Signal(
    name="task_postrun", providing_args={"task_id", "task", "args", "kwargs", "retval"}
)
...
task_failure = Signal(
    name="task_failure",
    providing_args={"task_id", "exception", "args", "kwargs", "traceback", "einfo"},
)
task_revoked = Signal(
    name="task_revoked", providing_args={"request", "terminated", "signum", "expired"}
)
task_rejected = Signal(name="task_rejected", providing_args={"message", "exc"})
task_unknown = Signal(
    name="task_unknown", providing_args={"message", "exc", "name", "id"}
)
...
celeryd_init = Signal(
    name="celeryd_init", providing_args={"instance", "conf", "options"}
)
celeryd_after_setup = Signal(
    name="celeryd_after_setup", providing_args={"instance", "conf"}
)
...
setup_logging = Signal(
    name="setup_logging", providing_args={"loglevel", "logfile", "format", "colorize"}
)
after_setup_logger = Signal(
    name="after_setup_logger",
    providing_args={"logger", "loglevel", "logfile", "format", "colorize"},
)
...
eventlet_pool_apply = Signal(
    name="eventlet_pool_apply", providing_args={"target", "args", "kwargs"}
)

# - Programs
user_preload_options = Signal(
    name="user_preload_options", providing_args={"app", "options"}
)
