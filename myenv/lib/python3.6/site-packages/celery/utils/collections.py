    ...
    def __getattr__(self, k):
        ...
        try:
            ...
            raise AttributeError(
                "{!r} object has no attribute {!r}".format(type(self).__name__, k)
            )
...
class ChainMap(MutableMapping):
    ...
    def __init__(self, *maps, **kwargs):
        ...
        self.__dict__.update(
            key_t=kwargs.get("key_t"), maps=maps, changes=maps[0], defaults=maps[1:]
        )
    ...
    def __delitem__(self, key):
        ...
        try:
            ...
            raise KeyError("Key not found in first mapping: {!r}".format(key))
...
class ConfigurationView(ChainMap, AttributeDictMixin):
    ...
    def __init__(self, changes, defaults=None, keys=None, prefix=None):
        ...
        self.__dict__.update(
            prefix=prefix.rstrip("_") + "_" if prefix else prefix, _keys=keys
        )
            ...
            for key, inserted in items(other):
                ...
                if not isinstance(inserted, float):
                    raise ValueError(
                        ...
                        "{!r} with value: {}".format(type(inserted), inserted)
                    )
...
class Messagebuffer(Evictable):
    ...
    def __repr__(self):
        # type: () -> str
        return "<{}: {}/{}>".format(type(self).__name__, len(self), self.maxsize)
...
class BufferMap(OrderedDict, Evictable):
    ...
    def __repr__(self):
        # type: () -> str
        return "<{}: {}/{}>".format(type(self).__name__, self.total, self.maxsize)
