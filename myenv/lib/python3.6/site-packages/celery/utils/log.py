...
def logger_isa(l, p, max=1000):
    ...
    for _ in range(max):
        if this == p:
            ...
            if this in seen:
                raise RuntimeError("Logger {!r} parents recursive".format(l.name))
            seen.add(this)
            this = this.parent
            if not this:
                break
    else:  # pragma: no cover
        raise RuntimeError("Logger hierarchy exceeds {}".format(max))
    return False
...
def get_task_logger(name):
    ...
    if name in RESERVED_LOGGER_NAMES:
        raise RuntimeError("Logger name {!r} is reserved!".format(name))
