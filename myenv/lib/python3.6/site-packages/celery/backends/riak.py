...
class RiakBackend(KeyValueStoreBackend):
        ...
        **kwargs
        ...
        if not riak:
            raise ImproperlyConfigured(
                "You need to install the riak library to use the " "Riak backend."
            )
    ...
    def _get_client(self):
        ...
        if self._client is None or not self._client.is_alive():
            self._client = RiakClient(
                protocol=self.protocol, host=self.host, pb_port=self.port
            )
    ...
    def _get_bucket(self):
        ...
        if self._client is None or not self._client.is_alive() or not self._bucket:
