...
class Backend(object):
    ...
    def store_result(
        ...
        self._store_result(task_id, result, state, traceback, request=request, **kwargs)
        return result
...
class BaseKeyValueStoreBackend(Backend):
    ...
    def get_key_for_task(self, task_id, key=""):
        ...
        key_t = self.key_t
        return key_t("").join([self.task_keyprefix, key_t(task_id), key_t(key)])

    def get_key_for_group(self, group_id, key=""):
        """Get the cache key for a group by id."""
        key_t = self.key_t
        return key_t("").join([self.group_keyprefix, key_t(group_id), key_t(key)])

    def get_key_for_chord(self, group_id, key=""):
        """Get the cache key for the chord waiting on group with given id."""
        key_t = self.key_t
        return key_t("").join([self.chord_keyprefix, key_t(group_id), key_t(key)])
    ...
    def get_many(
        ...
        while ids:
            ...
            if timeout and iterations * interval >= timeout:
                raise TimeoutError("Operation timed out ({})".format(timeout))
            if on_interval:
                on_interval()
    ...
    def _save_group(self, group_id, result):
        self.set(
            self.get_key_for_group(group_id), self.encode({"result": result.as_tuple()})
        )
        return result
    ...
    def on_chord_part_return(self, request, state, result, **kwargs):
        ...
        try:
            ...
            return self.chord_error_from_stack(
                callback, ChordError("Cannot restore group: {!r}".format(exc))
            )
        if deps is None:
            try:
                ...
                return self.chord_error_from_stack(
                    callback, ChordError("GroupResult {} no longer exists".format(gid))
                )
        val = self.incr(key)
        size = len(deps)
            ...
            try:
                ...
                try:
                    ...
                    self.chord_error_from_stack(
                        callback, ChordError("Callback error: {!r}".format(exc))
                    )
            finally:
                deps.delete()
