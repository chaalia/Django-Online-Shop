...
class AMQPBackend(BaseBackend):
    ...
    def wait_for(
        ...
        **kwargs
        ...
        if cache and cached_meta and cached_meta["status"] in READY_STATES:
            return cached_meta
        try:
            return self.consume(
                task_id, timeout=timeout, no_ack=no_ack, on_interval=on_interval
            )
    ...
    def get_task_meta(self, task_id, backlog_limit=1000):
        ...
        with self.app.pool.acquire_channel(block=True) as (_, channel):
            ...
            for i in range(backlog_limit):  # spool ffwd
                acc = binding.get(accept=self.accept, no_ack=False)
    ...
    def consume(self, task_id, timeout=None, no_ack=True, on_interval=None):
        ...
        with self.app.pool.acquire_channel(block=True) as (conn, channel):
            ...
            with self.Consumer(
                channel, binding, no_ack=no_ack, accept=self.accept
            ) as consumer:
                while 1:
                    try:
    ...
    def get_many(
        ...
        on_interval=None,
        now=monotonic,
        getfields=itemgetter("status", "task_id"),
        READY_STATES=states.READY_STATES,
        PROPAGATE_STATES=states.PROPAGATE_STATES,
        **kwargs
    ):
        with self.app.pool.acquire_channel(block=True) as (conn, channel):
            ...
            def _on_message(message):
                ...
                if state in READY_STATES:
                    push_result(body) if uid in task_ids else push_cache(uid, body)
