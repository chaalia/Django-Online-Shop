...
class RedisBackend(BaseKeyValueStoreBackend, AsyncBackendMixin):
    ...
    def _unpack_chord_result(
        ...
        if state in PROPAGATE_STATES:
            raise ChordError("Dependency {} raised {!r}".format(tid, retval))
        return retval
    ...
    def on_chord_part_return(self, request, state, result, propagate=None, **kwargs):
        ...
        with client.pipeline() as pipe:
            pipeline = (
                pipe.rpush(jkey, self.encode([1, tid, state, result]))
                .llen(jkey)
                .get(tkey)
            )
        ...
        try:
            ...
            if readycount == total:
                ...
                with client.pipeline() as pipe:
                    resl, = pipe.lrange(jkey, 0, total).execute()
                try:
            ...
            logger.exception("Chord %r raised: %r", request.group, exc)
            return self.chord_error_from_stack(
                callback, ChordError("Join error: {!r}".format(exc))
            )
...
class SentinelBackend(RedisBackend):
    ...
    def _get_sentinel_instance(self, **params):
        ...
        sentinel_kwargs = result_backend_transport_opts.get("sentinel_kwargs", {})
    ...
    def _get_pool(self, **params):
        ...
        return sentinel_instance.master_for(
            service_name=master_name, redis_class=self._get_client()
        ).connection_pool
