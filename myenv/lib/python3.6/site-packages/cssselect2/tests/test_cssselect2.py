...
def get_test_document():
    ...
    for id in ("any-namespace", "no-namespace"):
        ...
        div.set("id", id)
        etree.SubElement(div, "{http://www.w3.org/1999/xhtml}div").set(
            "id", id + "-div1"
        )
        etree.SubElement(div, "{http://www.w3.org/1999/xhtml}div").set(
            "id", id + "-div2"
        ...
        etree.SubElement(div, "{http://www.example.org/ns}div").set("id", id + "-div4")
...
def test_select():
    ...
    def select_ids(selector, html_only):
        xml_ids = [
            ...
            for element in ElementWrapper.from_xml_root(root).query_all(selector)
        ]
        html_ids = [
            element.etree_element.get("id", "nil")
            for element in ElementWrapper.from_html_root(root).query_all(selector)
        ]
        if html_only:
            assert xml_ids == []
        else:
    ...
    assert pcss('*[lang|="e"]') == []
    # ... :lang() is not.
    assert pcss(":lang(EN)", "*:lang(en-US)" ":lang(En)") == ["second-li", "li-div"]
    ...
    assert pcss(":link") == ["link-href", "tag-anchor", "nofollow-anchor", "area-href"]
    ...
    assert pcss(":checked") == ["checkbox-checked", "checkbox-disabled-checked"]
