    ...
    def __init__(self, parsed_selector):
        ...
        for simple_selector in node.simple_selectors:
            if isinstance(simple_selector, parser.IDSelector):
                ...
                isinstance(simple_selector, parser.AttributeSelector)
                and simple_selector.name == "lang"
            ):
...
def _compile_node(selector):
    ...
    if isinstance(selector, parser.CombinedSelector):
        ...
        # so that 'left_inside' applies to different elements.
            ...
            left = "next(el is not None and (%s) for el in [el.parent])" % left_inside
        elif selector.combinator == "+":
            left = "next(el is not None and (%s) for el in [el.previous])" % left_inside
        elif selector.combinator == "~":
            left = "any((%s) for el in el.iter_previous_siblings())" % left_inside
        else:
            raise SelectorError("Unknown combinator", selector.combinator)
        ...
        return "el.local_name == ({!r} if el.in_html_document else {!r})".format(
        ...
        if selector.namespace is not None:
            if selector.namespace:
                key = "({!r} if el.in_html_document else {!r})".format(
                    "{{{}}}{}".format(selector.namespace, selector.lower_name),
                    "{{{}}}{}".format(selector.namespace, selector.name),
                )
            else:
                key = "({!r} if el.in_html_document else {!r})".format(
            ...
            if selector.operator is None:
                ...
                return "el.etree_element.get({}) == {!r}".format(key, value)
            elif selector.operator == "~=":
                if len(value.split()) != 1 or value.strip() != value:
                    return "0"
                else:
                    return '{!r} in split_whitespace(el.etree_element.get({}, ""))'.format(
                        value, key
                    )
            elif selector.operator == "|=":
                return (
                    "next(v == %r or (v is not None and v.startswith(%r))"
                    "     for v in [el.etree_element.get(%s)])"
                    % (value, value + "-", key)
                )
            elif selector.operator == "^=":
                if value:
                    return 'el.etree_element.get({}, "").startswith({!r})'.format(
                        key, value
                    )
                else:
                    return "0"
                ...
                if value:
                    return 'el.etree_element.get({}, "").endswith({!r})'.format(
                        key, value
                    )
                else:
                    return "0"
                ...
                if value:
                    return '{!r} in el.etree_element.get({}, "")'.format(value, key)
                else:
                    return "0"
            else:
        ...
        if selector.name == "link":
            return '%s and el.etree_element.get("href") is not None' % html_tag_eq(
                "a", "area", "link"
            )
            ...
            return (
                ...
                % (
                    html_tag_eq("button", "input", "select", "textarea", "option"),
            ...
            return (
                ...
                % (html_tag_eq("input", "menuitem"), html_tag_eq("option"))
            )
        elif selector.name in ("visited", "hover", "active", "focus", "target"):
        ...
        if selector.name == "lang":
            ...
            if result is None:
                raise SelectorError("Invalid arguments for :%s()" % selector.name)
            a, b = result
            ...
            if a == 0:
                ...
                return (
                    "next(r == 0 and n >= 0"
                    "     for n, r in [divmod(%s - %i, %i)])" % (count, B, a)
                )
...
def html_tag_eq(*local_names):
    if len(local_names) == 1:
        return (
            ...
            % (local_names[0], "{http://www.w3.org/1999/xhtml}" + local_names[0])
        )
    else:
        return (
