...
if HIREDIS_AVAILABLE:
    ...
    if not HIREDIS_SUPPORTS_BYTE_BUFFER:
        msg = (
            "redis-py works best with hiredis >= 0.1.4. You're running "
    ...
    def _read_from_socket(self, length=None, timeout=SENTINEL, raise_on_timeout=True):
        ...
        try:
            ...
            if not raise_on_timeout and ex.errno == allowed:
                return False
            raise ConnectionError("Error while reading from socket: {}".format(ex.args))
        finally:
            if custom_timeout:
...
class Connection(object):
    ...
    def __repr__(self):
        repr_args = ",".join(["{}={}".format(k, v) for k, v in self.repr_pieces()])
        return "{}<{}>".format(self.__class__.__name__, repr_args)
    ...
    def _error_message(self, exception):
        ...
        if len(exception.args) == 1:
            return "Error connecting to {}:{}. {}.".format(
    ...
    def send_packed_command(self, command, check_health=True):
        ...
        try:
            ...
            raise ConnectionError(
                "Error {} while writing to socket. {}.".format(errno, errmsg)
            )
    ...
    def read_response(self):
        ...
        try:
            ...
            self.disconnect()
            raise TimeoutError(
                "Timeout reading from {}:{}".format(self.host, self.port)
            )
        except socket.error as e:
            self.disconnect()
            raise ConnectionError(
                "Error while reading from {}:{} : {}".format(
        ...
        **kwargs
        ...
        if ssl_cert_reqs is None:
            ...
            if ssl_cert_reqs not in CERT_REQS:
                raise RedisError(
                    "Invalid SSL Certificate Requirements Flag: %s" % ssl_cert_reqs
                )
...
class UnixDomainSocketConnection(Connection):
    ...
    def _error_message(self, exception):
        ...
        # or just "message"
        if len(exception.args) == 1:
            return "Error connecting to unix socket: {}. {}.".format(
...
class ConnectionPool(object):
    ...
    def __repr__(self):
        return "{}<{}>".format(
            type(self).__name__, repr(self.connection_class(**self.connection_kwargs))
        )
