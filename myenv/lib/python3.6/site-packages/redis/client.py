...
from redis.connection import ConnectionPool, UnixDomainSocketConnection, SSLConnection
...
class Redis(object):
    ...
    RESPONSE_CALLBACKS = dict_merge(
        ...
        string_keys_to_dict("BGREWRITEAOF BGSAVE", lambda r: True),
        {
            ...
            "GEOPOS": lambda r: list(
                map(
                    lambda ll: (float(ll[0]), float(ll[1])) if ll is not None else None,
                    r,
                )
            ),
    ...
    def client_kill_filter(self, _id=None, _type=None, addr=None, skipme=None):
        ...
        if _type is not None:
            ...
            if str(_type).lower() not in client_types:
                raise DataError(
                    "CLIENT KILL type must be one of {!r}".format(client_types)
                )
    ...
    def client_list(self, _type=None):
        ...
        if _type is not None:
            ...
            if str(_type).lower() not in client_types:
                raise DataError(
                    "CLIENT LIST _type must be one of {!r}".format(client_types)
                )
...
class PubSub(object):
    ...
    def __init__(
        self, connection_pool, shard_hint=None, ignore_subscribe_messages=False
    ):
        self.connection_pool = connection_pool
