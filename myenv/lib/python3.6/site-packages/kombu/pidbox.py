...
class Mailbox(object):
    ...
    def get_reply_queue(self):
        oid = self.oid
        return Queue(
            "{}.{}".format(oid, self.reply_exchange.name),
    ...
    def get_queue(self, hostname):
        return Queue(
            "{}.{}.pidbox".format(hostname, self.namespace),
        ...
        with self.producer_or_acquire(producer, chan) as producer:
            try:
                producer.publish(
                    ...
                    declare=[exchange],
                    headers={"ticket": ticket, "clock": self.clock.forward()},
                    retry=True,
                    **opts
                )
    ...
    def _broadcast(
        ...
        if (
            pattern is not None
            and not isinstance(pattern, string_t)
            and matcher is not None
            and not isinstance(matcher, string_t)
        ):
            raise ValueError(
