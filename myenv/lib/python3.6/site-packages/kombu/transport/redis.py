...
class Channel(virtual.Channel):
    ...
    def _do_restore_message(
        self, payload, exchange, routing_key, client=None, leftmost=False
    ):
        with self.conn_or_acquire(client) as client:
            try:
                ...
                for queue in self._lookup(exchange, routing_key):
                    (client.lpush if leftmost else client.rpush)(queue, dumps(payload))
            except Exception:
    ...
    def _restore(self, message, leftmost=False):
        ...
        with self.conn_or_acquire() as client:
            with client.pipeline() as pipe:
                P, _ = (
                    pipe.hget(self.unacked_key, tag)
    ...
    def _handle_message(self, client, r):
        ...
        return {"type": type, "pattern": pattern, "channel": channel, "data": data}
    ...
    def _put_fanout(self, exchange, message, routing_key, **kwargs):
        ...
        with self.conn_or_acquire() as client:
            client.publish(
                self._get_publish_topic(exchange, routing_key), dumps(message)
            )
    ...
    def _prepare_virtual_host(self, vhost):
        if not isinstance(vhost, numbers.Integral):
            ...
            try:
                ...
                raise ValueError(
                    "Database is int between 0 and limit - 1, not {}".format(vhost)
                )
        return vhost
...
class SentinelChannel(Channel):
    ...
    def _sentinel_managed_pool(self, asynchronous=False):
        ...
        return sentinel_inst.master_for(master_name, self.Client).connection_pool
