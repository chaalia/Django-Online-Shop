...
class Channel(virtual.Channel):
    ...
    def _lock_key(self, queue):
        return "{}/{}.lock".format(self.prefix, queue)

    def _key_prefix(self, queue):
        return "{}/{}".format(self.prefix, queue)
    ...
    def _put(self, queue, payload, **_):
        ...
        """
        key = "{}/msg/{}_{}".format(
            self._key_prefix(queue), int(round(monotonic() * 1000)), uuid.uuid4()
        )
        if not self.client.kv.put(key=key, value=dumps(payload), cas=0):
            raise ChannelError("Cannot add key {!r} to consul".format(key))
    ...
    def _get(self, queue, timeout=None):
        ...
        with self._queue_lock(queue, raising=Empty):
            ...
            self.index, data = self.client.kv.get(
                key=key, recurse=True, index=self.index, wait=self.timeout
            )
    ...
    def _purge(self, queue):
        ...
        return self.client.kv.delete(
            key="{}/msg/".format(self._key_prefix(queue)), recurse=True
        )
...
class Transport(virtual.Transport):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.connection_errors = virtual.Transport.connection_errors + (
            consul.ConsulException,
            consul.base.ConsulException,
        )

        self.channel_errors = virtual.Transport.channel_errors + (
            consul.ConsulException,
            consul.base.ConsulException,
        )
