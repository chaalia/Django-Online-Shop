...
class Channel(virtual.Channel):
    ...
    def _queue_lock(self, queue):
        ...
        logger.debug("Acquiring lock {}".format(lock.name))
        lock.acquire(blocking=True, lock_ttl=self.lock_ttl)
        try:
            yield
        finally:
            logger.debug("Releasing lock {}".format(lock.name))
            lock.release()
    ...
    def _new_queue(self, queue, **_):
        ...
        with self._queue_lock(queue):
            try:
                ...
                logger.debug('Queue "{}" already exists'.format(queue))
    ...
    def _put(self, queue, payload, **_):
        ...
        with self._queue_lock(queue):
            ...
            if not self.client.write(key=key, value=dumps(payload), append=True):
                raise ChannelError("Cannot add key {!r} to etcd".format(key))
    ...
    def _purge(self, queue):
        ...
        with self._queue_lock(queue):
            key = self._key_prefix(queue)
            logger.debug("Purging queue at key {}".format(key))
...
class Transport(virtual.Transport):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.channel_errors = virtual.Transport.channel_errors + (etcd.EtcdException,)
