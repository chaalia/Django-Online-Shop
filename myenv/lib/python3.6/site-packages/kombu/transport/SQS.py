...
class Channel(virtual.Channel):
    ...
    def _reset_cycle(self):
        ...
        """
        self._cycle = scheduling.FairCycle(self._get_bulk, self._active_queues, Empty)
    ...
    def _create_queue(self, queue_name, attributes):
        ...
        attributes.update(self.transport_options.get("sqs-creation-attributes") or {})
    ...
    def _message_to_python(self, message, queue_name, queue):
        ...
        if queue_name in self._noack_queues:
            ...
            try:
                ...
                payload.update({"body": bytes_to_str(body), "properties": properties})
    ...
    def _schedule_queue(self, queue):
        if queue in self._active_queues:
            if self.qos.can_consume():
                self._get_bulk_async(queue, callback=promise(self._loop1, (queue,)))
            else:
                self._loop1(queue)
    ...
    def _get_message_estimate(self, max_if_unlimited=SQS_MAX_MESSAGES):
        ...
        return min(
            max_if_unlimited if maxcount is None else max(maxcount, 1), max_if_unlimited
        )
