...
class CurlClient(BaseClient):
    ...
    def __init__(self, hub=None, max_clients=10):
        ...
        self._timeout_check_tref = self.hub.call_repeatedly(1.0, self._timeout_check)
    ...
    def _setup_request(self, curl, request, buffer, headers, _pycurl=pycurl):
        ...
        setopt(
            _pycurl.HTTPHEADER, ["{}: {}".format(*h) for h in items(request.headers)]
        )
        ...
        setopt(
            _pycurl.USERAGENT, bytes_to_str(request.user_agent or DEFAULT_USER_AGENT)
        )
