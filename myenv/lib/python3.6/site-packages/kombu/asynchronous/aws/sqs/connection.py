...
class AsyncSQSConnection(AsyncAWSQueryConnection):
    ...
    def __init__(self, sqs_connection, debug=0, region=None, **kwargs):
        ...
        AsyncAWSQueryConnection.__init__(
            self, sqs_connection, region_name=region, debug=debug, **kwargs
        )

    def create_queue(self, queue_name, visibility_timeout=None, callback=None):
        params = {"QueueName": queue_name}
        if visibility_timeout:
            params["DefaultVisibilityTimeout"] = format(visibility_timeout, "d")
    ...
    def delete_message(self, queue, receipt_handle, callback=None):
        return self.delete_message_from_handle(queue, receipt_handle, callback)
            ...
            params.update(
                {
                    ...
                    "{}.ReceiptHandle".format(prefix): m.receipt_handle,
                }
            )
        return self.get_object(
            "DeleteMessageBatch", params, queue.id, verb="POST", callback=callback
        )

    def delete_message_from_handle(self, queue, receipt_handle, callback=None):
        return self.get_status(
            "DeleteMessage", {"ReceiptHandle": receipt_handle}, queue, callback=callback
        )
    ...
    def send_message(self, queue, message_content, delay_seconds=None, callback=None):
        ...
        return self.get_object(
            "SendMessage", params, queue.id, verb="POST", callback=callback
        )
    ...
    def send_message_batch(self, queue, messages, callback=None):
        ...
        for i, msg in enumerate(messages):
            ...
            params.update(
                {
                    "{}.Id".format(prefix): msg[0],
                    "{}.MessageBody".format(prefix): msg[1],
                    "{}.DelaySeconds".format(prefix): msg[2],
                }
            )
        return self.get_object(
            "SendMessageBatch", params, queue.id, verb="POST", callback=callback
        )
    ...
    def change_message_visibility_batch(self, queue, messages, callback=None):
        ...
        for i, t in enumerate(messages):
            ...
            params.update(
                {
                    "{}.Id".format(pre): t[0].id,
                    "{}.ReceiptHandle".format(pre): t[0].receipt_handle,
                    "{}.VisibilityTimeout".format(pre): t[1],
                }
            )
        return self.get_object(
    ...
    def get_all_queues(self, prefix="", callback=None):
        ...
        return self.get_list(
            "ListQueues", params, [("QueueUrl", AsyncQueue)], callback=callback
        )

    def get_queue(self, queue_name, callback=None):
        # TODO Does not support owner_acct_id argument
        return self.get_all_queues(
            queue_name, transform(self._on_queue_ready, callback, queue_name)
        )
