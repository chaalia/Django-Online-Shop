...
from .five import bytes_if_py2, python_2_unicode_compatible, reraise, string_t, text_t
...
class Connection(object):
        ...
        if hostname:
            ...
            if "://" in hostname and "+" in hostname[: hostname.index("://")]:
                ...
                if not get_transport_cls(transport).can_parse_url:
                    ...
                    params.update(
                        dictfilter(url_params), hostname=url_params["hostname"]
                    )
    ...
    def _debug(self, msg, *args, **kwargs):
        if self._logger:  # pragma: no cover
            ...
            logger.debug(fmt.format(id=id(self), msg=text_t(msg)), *args, **kwargs)
    ...
    def _reraise_as_library_errors(
        ...
        try:
            ...
            reraise(ConnectionError, ConnectionError(text_t(exc)), sys.exc_info()[2])
        except self.recoverable_channel_errors as exc:
            reraise(ChannelError, ChannelError(text_t(exc)), sys.exc_info()[2])
    ...
    def ensure(
        ...
        _ensured.__name__ = bytes_if_py2("{}(ensured)".format(fun.__name__))
    ...
    def _info(self, resolve=True):
        ...
        if self.uri_prefix:
            hostname = "{}+{}".format(self.uri_prefix, hostname)
    ...
    def as_uri(
        ...
        if self.transport.can_parse_url:
            ...
            if self.uri_prefix:
                connection_as_uri = "{}+{}".format(self.uri_prefix, hostname)
