...
class Exchange(MaybeChannelBound):
    ...
    def __hash__(self):
        return hash("E|{}".format(self.name))
    ...
    def Message(self, body, delivery_mode=None, properties=None, **kwargs):
        ...
        if (
            isinstance(body, string_t) and properties.get("content_encoding", None)
        ) is None:
    ...
    def __str__(self):
        return "Exchange {}({})".format(_reprstr(self.name) or repr(""), self.type)
...
class Queue(MaybeChannelBound):
    ...
    def __hash__(self):
        return hash("Q|{}".format(self.name))
        ...
        if self.bindings:
            return self._repr_entity(
                "Queue {name} -> {bindings}".format(
                    name=_reprstr(self.name), bindings=pretty_bindings(self.bindings)
                )
            )
        return self._repr_entity(
            "Queue {name} -> {0.exchange!r} -> {routing_key}".format(
                self, name=_reprstr(self.name), routing_key=_reprstr(self.routing_key)
            )
        )
