...
class SerializerRegistry(object):
    ...
    def register(self, name, encoder, decoder, content_type, content_encoding="utf-8"):
        ...
        """
        if encoder:
            self._encoders[name] = codec(content_type, content_encoding, encoder)
        if decoder:
    ...
    def unregister(self, name):
        ...
        try:
            ...
            raise SerializerNotInstalled(
                "No encoder/decoder installed for {}".format(name)
            )
    ...
    def _set_default_serializer(self, name):
        ...
        try:
            ...
            raise SerializerNotInstalled("No encoder installed for {}".format(name))
    ...
    def dumps(self, data, serializer=None):
        ...
        if serializer and not self._encoders.get(serializer):
            raise SerializerNotInstalled(
                "No encoder installed for {}".format(serializer)
            )
        ...
        if data:
            ...
            if decode:
                with _reraise_errors(DecodeError):
                    return decode(data)
            if content_encoding not in SKIP_DECODE and not isinstance(data, text_t):
                with _reraise_errors(DecodeError):
    ...
    def _for_untrusted_content(self, ctype, why):
        return ContentDisallowed(
            "Refusing to deserialize {} content of type {}".format(
                why, parenthesize_alias(self.type_to_name.get(ctype, ctype), ctype)
            )
        )
