...
class FieldExpr:
    ...
    def as_c_expr(self):
        ...
        return (
            '  {{ "{}", {},\n'.format(self.name, self.field_offset)
            + "     {}   {},\n".format(spaces, self.field_size)
            + "     {}   {} }},".format(spaces, self.field_type_op.as_c_expr())
        )
    ...
    def as_field_python_expr(self):
        if self.field_type_op.op == OP_NOOP:
            ...
            raise NotImplementedError
        return "b'{}{}{}'".format(
            self.field_type_op.as_python_bytes(), size_expr, self.name
        )
...
class StructUnionExpr:
    ...
    def as_c_expr(self):
        return (
            ...
            + "\n    {}, {}, ".format(self.size, self.alignment)
...
class TypenameExpr:
    ...
    def as_python_expr(self):
        return "b'{}{}'".format(format_four_bytes(self.type_index), self.name)
...
class Recompiler:
    ...
    def _generate(self, step_name):
        ...
        for name, (tp, quals) in sorted(lst):
            ...
            try:
                method = getattr(self, "_generate_cpy_{}_{}".format(kind, step_name))
    ...
    def write_c_source_to_f(self, f, preamble):
        ...
        if self.ffi._embedding is not None:
            prnt('#define _CFFI_MODULE_NAME  "{}"'.format(self.module_name))
            prnt("static const char _CFFI_PYTHON_STARTUP_CODE[] = {")
            self._print_string_literal_in_array(self.ffi._embedding)
            prnt("0 };")
            prnt("#ifdef PYPY_VERSION")
            prnt(
                "# define _CFFI_PYTHON_STARTUP_FUNC  _cffi_pypyinit_{}".format(
                    base_module_name
                )
            )
            prnt("#elif PY_MAJOR_VERSION >= 3")
            prnt(
                "# define _CFFI_PYTHON_STARTUP_FUNC  PyInit_{}".format(base_module_name)
            )
            prnt("#else")
            prnt("# define _CFFI_PYTHON_STARTUP_FUNC  init{}".format(base_module_name))
            prnt("#endif")
        ...
        for step_name in self.ALL_STEPS:
            ...
            if nums[step_name] > 0:
                prnt(
                    "static const struct _cffi_{}_s _cffi_{}s[] = {{".format(
                        step_name, step_name
                    )
                )
                for entry in lst:
        ...
        if self.ffi._included_ffis:
            ...
            for ffi_to_include in self.ffi._included_ffis:
                ...
                prnt('  "{}",'.format(included_module_name))
            prnt("  NULL")
            prnt("};")
            prnt()
        #
        ...
        prnt("#ifdef PYPY_VERSION")
        prnt("PyMODINIT_FUNC")
        prnt("_cffi_pypyinit_{}(const void *p[])".format(base_module_name))
        prnt("{")
        if self._num_externpy:
        ...
        prnt("     PyMODINIT_FUNC")
        prnt("#  if PY_MAJOR_VERSION >= 3")
        prnt("     PyInit_{}(void) {{ return NULL; }}".format(base_module_name))
        prnt("#  else")
        prnt("     init{}(void) {{ }}".format(base_module_name))
        prnt("#  endif")
        prnt("#  endif")
        prnt("#elif PY_MAJOR_VERSION >= 3")
        prnt("PyMODINIT_FUNC")
        prnt("PyInit_{}(void)".format(base_module_name))
        prnt("{")
        prnt(
            '  return _cffi_init("{}", 0x{:x}, &_cffi_type_context);'.format(
        ...
        prnt("{")
        prnt(
            '  _cffi_init("{}", 0x{:x}, &_cffi_type_context);'.format(
                self.module_name, self._version
            )
        )
        prnt("}")
    ...
    def write_py_source_to_f(self, f):
        ...
        prnt()
        prnt("ffi = _cffi_backend.FFI('{}',".format(self.module_name))
        prnt("    _version = 0x{:x},".format(self._version))
        self._version = None
        #
        ...
        for step_name in self.ALL_STEPS:
            ...
            if len(lst) > 0 and step_name != "field":
                prnt("    _{}s = {},".format(step_name, self._to_py(lst)))
        #
    ...
    def _convert_funcarg_to_c(self, tp, fromvar, tovar, errcode):
        ...
        #
        self._prnt("  {} = {}({}{});".format(tovar, converter, fromvar, extraarg))
        self._prnt(
    ...
    def _convert_expr_from_c(self, tp, var, context):
        if isinstance(tp, model.BasePrimitiveType):
            if tp.is_integer_type() and tp.name != "_Bool":
                return "_cffi_from_c_int({}, {})".format(var, tp.name)
            elif isinstance(tp, model.UnknownFloatType):
                return "_cffi_from_c_double({})".format(var)
            elif tp.name != "long double" and not tp.is_complex_type():
                cname = tp.name.replace(" ", "_")
                if cname in ("char16_t", "char32_t"):
                    self.needs_version(VERSION_CHAR16CHAR32)
                return "_cffi_from_c_{}({})".format(cname, var)
            else:
    ...
    def _typedef_type(self, tp, name):
        return self._global_type(tp, "(*({} *)0)".format(name))
    ...
    def _generate_cpy_function_decl(self, tp, name):
        ...
        if tp.abi:
            ...
            abi = ""
        name_and_arguments = "{}_cffi_d_{}({})".format(abi, name, repr_arguments)
        prnt("static {}".format(tp.result.get_c_name(name_and_arguments)))
        prnt("{")
        call_arguments = ", ".join(call_arguments)
        result_code = "return "
        if isinstance(tp.result, model.VoidType):
            result_code = ""
        prnt("  {}{}({});".format(result_code, name, call_arguments))
        prnt("}")
        #
        prnt("#ifndef PYPY_VERSION")  # ------------------------------
        #
        prnt("static PyObject *")
        prnt("_cffi_f_{}(PyObject *self, PyObject *{})".format(name, argname))
        prnt("{")
        #
        context = "argument of %s" % name
        for i, type in enumerate(tp.args):
        ...
        if difference:
            repr_arguments = ", ".join(arguments)
            repr_arguments = repr_arguments or "void"
            name_and_arguments = "{}_cffi_f_{}({})".format(abi, name, repr_arguments)
            prnt("static {}".format(tp_result.get_c_name(name_and_arguments)))
            prnt("{")
            if result_decl:
                prnt(result_decl)
            call_arguments = ", ".join(call_arguments)
            ...
            prnt("#  define _cffi_f_{} _cffi_d_{}".format(name, name))
        #
        prnt("#endif")  # ------------------------------
        prnt()
    ...
    def _struct_decl(self, tp, cname, approxname):
        if tp.fldtypes is None:
            return
        prnt = self._prnt
        checkfuncname = "_cffi_checkfld_{}".format(approxname)
        prnt("_CFFI_UNUSED_FN")
        prnt("static void {}({} *p)".format(checkfuncname, cname))
        prnt("{")
        ...
        prnt("}")
        prnt("struct _cffi_align_{} {{ char x; {} y; }};".format(approxname, cname))
        prnt()
        ...
        if reason_for_not_expanding is None:
            ...
            for fldname, fldtype, fbitsize, fqual in enumfields:
                ...
                self._check_not_opaque(
                    fldtype, "field '{}.{}'".format(tp.name, fldname)
                )
            ...
            if cname is None:  # unknown name, for _add_missing_struct_unions
                ...
                if named_ptr is not None:
                    size = "sizeof(*({})0)".format(named_ptr.name)
                    align = "-1 /* unknown alignment */"
                else:
                    size = "sizeof({})".format(cname)
                    align = "offsetof(struct _cffi_align_{}, y)".format(approxname)
                comment = None
        else:
            size = "(size_t)-1"
            align = -1
        ...
        for tp, order in lst:
            if tp not in self._seen_struct_unions:
                ...
                if tp.name.startswith("$") and tp.name[1:].isdigit():
                    ...
                    raise NotImplementedError("internal inconsistency: {!r}".format(tp))
    ...
    def _generate_cpy_const(
        ...
        if (category, name) in self._seen_constants:
            raise VerificationError(
                "duplicate declaration of {} '{}'".format(category, name)
            )
        self._seen_constants.add((category, name))
        #
        prnt = self._prnt
        funcname = "_cffi_{}_{}".format(category, name)
        if is_int:
            prnt("static int %s(unsigned long long *o)" % funcname)
            prnt("{")
            prnt("  int n = ({}) <= 0;".format(name))
            prnt(
            ...
            if check_value is not None:
                ...
                prnt("  if (!_cffi_check_int(*o, n, {}))".format(check_value))
                prnt("    n |= 2;")
            prnt("  return n;")
            prnt("}")
        else:
    ...
    def _global_type(self, tp, global_name):
        if isinstance(tp, model.ArrayType):
            ...
            if actual_length == "...":
                actual_length = "_cffi_array_len({})".format(global_name)
    ...
    def _generate_cpy_variable_decl(self, tp, name):
        ...
        decl = "*_cffi_var_{}(void)".format(name)
        prnt("static " + tp.get_c_name(decl, quals=self._current_quals))
        prnt("{")
        prnt("  return {}({});".format(ampersand, name))
        prnt("}")
        prnt()
    ...
    def _extern_python_decl(self, tp, name, tag_and_space):
        ...
        prnt(
            '  {{ "{}.{}", {}, 0, 0 }};'.format(self.module_name, name, size_of_result)
        )
        prnt()
        #
        arguments = []
        ...
        repr_arguments = ", ".join(arguments)
        repr_arguments = repr_arguments or "void"
        name_and_arguments = "{}({})".format(name, repr_arguments)
        if tp.abi == "__stdcall":
        ...
        prnt("{}{}".format(tag_and_space, tp.result.get_c_name(name_and_arguments)))
        prnt("{")
    ...
    def _emit_bytecode_RawFunctionType(self, tp, index):
        ...
        if tp.abi is not None:
            if tp.abi == "__stdcall":
                ...
                raise NotImplementedError("abi={!r}".format(tp.abi))
        ...
        if call_c_compiler:
            ...
            try:
                ...
                if compiler_verbose:
                    ...
                    print("{} {!r}".format(msg, os.path.abspath(tmpdir)))
                os.chdir(tmpdir)
