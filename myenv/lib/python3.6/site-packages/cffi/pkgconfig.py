    ...
    for key, value in cfg2.items():
        if key not in cfg1:
            ...
            if not isinstance(cfg1[key], list):
                raise TypeError("cfg1[{!r}] should be a list of strings".format(key))
            if not isinstance(value, list):
                raise TypeError("cfg2[{!r}] should be a list of strings".format(key))
...
def call(libname, flag, encoding=sys.getfilesystemencoding()):
    ...
    try:
        ...
        raise PkgConfigError("cannot run pkg-config: {}".format(str(e).strip()))
