...
class VGenericEngine(object):
    ...
    def _generate(self, step_name):
        for name, tp in self._get_declarations():
            ...
            try:
                method = getattr(self, "_generate_gen_{}_{}".format(kind, step_name))
    ...
    def _load(self, module, step_name, **kwds):
        for name, tp in self._get_declarations():
            kind, realname = name.split(" ", 1)
            method = getattr(self, "_{}_gen_{}".format(step_name, kind))
            try:
    ...
    def _generate_gen_function_decl(self, tp, name):
        ...
        if tp.abi:
            ...
            abi = ""
        funcdecl = " {}{}({})".format(abi, wrappername, arglist)
    ...
    def _generate_struct_or_union_decl(self, tp, prefix, name):
        ...
        prnt("intptr_t {}(intptr_t i)".format(layoutfuncname))
        prnt("{")
    ...
    def _generate_gen_const(
        ...
        if check_value is not None:
            ...
            prnt("{")
            prnt("  *out_value = (long long)({});".format(name))
            prnt("  return ({}) <= 0;".format(name))
            prnt("}")
        else:
            ...
            if category == "const" and isinstance(tp, model.StructOrUnion):
                extra = "const *"
                ampersand = "&"
            prnt(tp.get_c_name(" {}{}(void)".format(extra, funcname), name))
            prnt("{")
    ...
    def _generate_gen_variable_decl(self, tp, name):
        if isinstance(tp, model.ArrayType):
            if tp.length == "...":
                prnt = self._prnt
                funcname = "_cffi_sizeof_{}".format(name)
                self.export_symbols.append(funcname)
                prnt("size_t %s(void)" % funcname)
                prnt("{")
                prnt("  return sizeof({});".format(name))
                prnt("}")
