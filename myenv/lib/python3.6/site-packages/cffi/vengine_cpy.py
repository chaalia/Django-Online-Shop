...
class VCPythonEngine(object):
    ...
    def write_source_to_f(self):
        ...
        prnt("  if (lib == NULL)")
        prnt("    return NULL;")
        prnt("  if ({} < 0 || _cffi_init() < 0) {{".format(constants))
        prnt("    Py_DECREF(lib);")
        prnt("    return NULL;")
        prnt("  }")
        prnt("  return lib;")
    ...
    def load_library(self, flags=None):
        ...
        try:
            ...
            try:
                ...
                error = "importing {!r}: {}".format(self.verifier.modulefilename, e)
    ...
    def _load(self, module, step_name, **kwds):
        for name, tp in self._get_declarations():
            kind, realname = name.split(" ", 1)
            method = getattr(self, "_{}_cpy_{}".format(step_name, kind))
            try:
        ...
        if isinstance(tp, model.PrimitiveType):
            if tp.is_integer_type() and tp.name != "_Bool":
                ...
                converter = "({})_cffi_to_c_{}".format(
                    tp.get_c_name(""), tp.name.replace(" ", "_")
                )
            errvalue = "-1"
        #
    ...
    def _generate_cpy_function_method(self, tp, name):
        ...
        if numargs == 0:
            ...
            meth = "METH_VARARGS"
        self._prnt('  {{"{}", _cffi_f_{}, {}, NULL}},'.format(name, name, meth))
    ...
    def _generate_struct_or_union_decl(self, tp, prefix, name):
        ...
        checkfuncname = "_cffi_check_{}_{}".format(prefix, name)
        layoutfuncname = "_cffi_layout_{}_{}".format(prefix, name)
        cname = ("{} {}".format(prefix, name)).strip()
        #
        prnt = self._prnt
        ...
        prnt("}")
        prnt("static PyObject *")
        prnt("{}(PyObject *self, PyObject *noarg)".format(layoutfuncname))
        prnt("{")
        ...
        for fname, ftype, fbitsize, fqual in tp.enumfields():
            ...
            prnt("    offsetof({}, {}),".format(cname, fname))
            if isinstance(ftype, model.ArrayType) and ftype.length is None:
                prnt("    0,  /* %s */" % ftype._get_c_name())
            else:
                prnt("    sizeof((({} *)0)->{}),".format(cname, fname))
        prnt("    -1")
        prnt("  };")
        prnt("  (void)self; /* unused */")
        prnt("  (void)noarg; /* unused */")
    ...
    def _generate_struct_or_union_method(self, tp, prefix, name):
        ...
        layoutfuncname = "_cffi_layout_{}_{}".format(prefix, name)
        self._prnt(
            '  {{"{}", {}, METH_NOARGS, NULL}},'.format(layoutfuncname, layoutfuncname)
        )
        ...
        if tp in self._struct_pending_verification:
            ...
            for fname, ftype, fbitsize, fqual in tp.enumfields():
                ...
                check(
                    ...
                    "wrong offset for field {!r}".format(fname),
                )
                if layout[i + 1] != 0:
                    ...
                    check(
                        layout[i + 1],
                        ffi.sizeof(BField),
                        "wrong size for field {!r}".format(fname),
                    )
                i += 2
            assert i == len(layout)
    ...
    def _generate_cpy_const(
        ...
        if size_too:
            ...
            prnt("    PyObject *o1 = o;")
            prnt(
                '    o = Py_BuildValue("On", o1, (Py_ssize_t)sizeof({}));'.format(name)
            )
            prnt("    Py_DECREF(o1);")
            prnt("    if (o == NULL)")
            prnt("      return -1;")
    ...
    def _enum_funcname(self, prefix, name):
        ...
        return "_cffi_e_{}_{}".format(prefix, name)
    ...
    def _generate_cpy_variable_decl(self, tp, name):
        if isinstance(tp, model.ArrayType):
            ...
            self._generate_cpy_const(
                False, name, tp, vartp=tp_ptr, size_too=(tp.length == "...")
            )
        else:
            tp_ptr = model.PointerType(tp)
