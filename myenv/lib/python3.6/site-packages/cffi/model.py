...
class BaseTypeByIdentity(object):
    ...
    def __repr__(self):
        return "<{}>".format(self._get_c_name())
...
class ArrayType(BaseType):
    ...
    def build_backend_type(self, ffi, finishlist):
        if self.length == "...":
            raise CDefError("cannot render the type {!r}: unknown length".format(self))
...
class StructOrUnionOrEnum(BaseTypeByIdentity):
    ...
    def build_c_name_with_marker(self):
        name = self.forcename or "{} {}".format(self.kind, self.name)
        self.c_name_with_marker = name + "&"
...
def global_cache(srctype, ffi, funcname, *args, **kwds):
    ...
    try:
        ...
        raise NotImplementedError("{}: {!r}: {}".format(funcname, srctype, e))
...
def attach_exception_info(e, name):
    if e.args and type(e.args[0]) is str:
        e.args = ("{}: {}".format(name, e.args[0]),) + e.args[1:]
