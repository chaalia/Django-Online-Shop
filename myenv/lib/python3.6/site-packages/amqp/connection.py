...
AMQP_HEARTBEAT_LOGGER = logging.getLogger("amqp.connection.Connection.heartbeat_tick")

#: Default map for :attr:`Connection.library_properties`
LIBRARY_PROPERTIES = {"product": "py-amqp", "product_version": __version__}
...
class Connection(AbstractChannel):
    ...
    _ALLOWED_METHODS_WHEN_CLOSING = (spec.Connection.Close, spec.Connection.CloseOk)

    connection_errors = (ConnectionError, socket.error, IOError, OSError)
    ...
    recoverable_channel_errors = (RecoverableChannelError,)
    ...
    def connect(self, callback=None):
        ...
        try:
            ...
            self.on_inbound_frame = self.frame_handler_cls(self, self.on_inbound_method)
    ...
    def _on_start(
        ...
        for authentication in self.authentication:
            ...
            raise ConnectionError(
                ...
                "(can offer: {}; available: {})".format(
                    b", ".join(
                        m.mechanism for m in self.authentication if m.mechanism
                    ).decode(),
        ...
        self.send_method(
            ...
            argsig,
            (client_properties, authentication.mechanism, login_response, self.locale),
        )
    ...
    def _on_tune_sent(self, argsig="ssb"):
        self.send_method(spec.Connection.Open, argsig, (self.virtual_host, "", False))
    ...
    def collect(self):
        try:
            ...
            temp_list = [x for x in values(self.channels or {}) if x is not self]
            for ch in temp_list:
                ch.collect()
        except socket.error:
    ...
    def _on_close(self, reply_code, reply_text, class_id, method_id):
        ...
        raise error_for_code(
            reply_code, reply_text, (class_id, method_id), ConnectionError
        )
    ...
    def heartbeat_tick(self, rate=2):
        ...
        AMQP_HEARTBEAT_LOGGER.debug(
            "heartbeat_tick : for connection %s", self._connection_id
        )
        ...
        if (
            ...
            and self.last_heartbeat_received + 2 * self.heartbeat < monotonic()
        ):
