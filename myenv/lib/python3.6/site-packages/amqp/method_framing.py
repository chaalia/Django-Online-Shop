...
_CONTENT_METHODS = frozenset([spec.Basic.Return, spec.Basic.Deliver, spec.Basic.GetOk])
...
def frame_handler(
    connection, callback, unpack_from=unpack_from, content_methods=_CONTENT_METHODS
):
    ...
    def write_frame(type_, channel, method_sig, args, content):
        ...
        if content:
            ...
            if isinstance(body, text_t):
                encoding = content.properties.setdefault("content_encoding", "utf-8")
                body = body.encode(encoding)
            properties = content._serialize_properties()
            bodylen = len(body)
            framelen = len(args) + (len(properties) or 0) + bodylen + FRAME_OVERHEAD
        ...
        if bigbody:
            ...
            frame = (
                b"".join([pack(">HH", *method_sig), args]) if type_ == 1 else b""
            )  # encode method frame
            framelen = len(frame)
            write(pack(">BHI%dsB" % framelen, type_, channel, framelen, frame, 0xCE))
            if body:
                frame = b"".join(
                    [pack(">HHQ", method_sig[0], 0, len(body)), properties]
                )
                framelen = len(frame)
                write(pack(">BHI%dsB" % framelen, 2, channel, framelen, frame, 0xCE))

                for i in range(0, bodylen, chunk_size):
                    frame = body[i : i + chunk_size]
                    framelen = len(frame)
                    write(
                        pack(">BHI%dsB" % framelen, 3, channel, framelen, frame, 0xCE)
                    )

        else:
            # ## FAST: pack into buffer and single write
            frame = b"".join([pack(">HH", *method_sig), args]) if type_ == 1 else b""
            framelen = len(frame)
            pack_into(
                ">BHI%dsB" % framelen,
                buf,
