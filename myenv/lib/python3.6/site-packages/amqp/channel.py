...
class Channel(AbstractChannel):
    ...
    _ALLOWED_METHODS_WHEN_CLOSING = (spec.Channel.Close, spec.Channel.CloseOk)

    def __init__(self, connection, channel_id=None, auto_decode=True, on_open=None):
        if channel_id:
    ...
    def close(self, reply_code=0, reply_text="", method_sig=(0, 0), argsig="BsBB"):
        ...
        try:
            is_closed = (
                ...
                or self.connection.channels is None
            )
            if is_closed:
                return
    ...
    def exchange_delete(self, exchange, if_unused=False, nowait=False, argsig="Bsbb"):
        """Delete an exchange.
    ...
    def queue_declare(
        ...
        self.send_method(
            spec.Queue.Declare,
            argsig,
            (0, queue, passive, durable, exclusive, auto_delete, nowait, arguments),
        )
        if not nowait:
    ...
    def queue_delete(
        self, queue="", if_unused=False, if_empty=False, nowait=False, argsig="Bsbbb"
    ):
        """Delete a queue.
    ...
    def basic_ack(self, delivery_tag, multiple=False, argsig="Lb"):
        ...
        """
        return self.send_method(spec.Basic.Ack, argsig, (delivery_tag, multiple))
    ...
    def basic_consume(
        ...
        p = self.send_method(
            spec.Basic.Consume,
            argsig,
            (0, queue, consumer_tag, no_local, no_ack, exclusive, nowait, arguments),
    ...
    def _on_basic_deliver(
        self, consumer_tag, delivery_tag, redelivered, exchange, routing_key, msg
    ):
        msg.channel = self
        msg.delivery_info = {
    ...
    def _on_get_ok(
        self, delivery_tag, redelivered, exchange, routing_key, message_count, msg
    ):
        msg.channel = self
        msg.delivery_info = {
            "delivery_tag": delivery_tag,
    ...
    def basic_qos(self, prefetch_size, prefetch_count, a_global, argsig="lBb"):
        ...
        PARAMETERS:
            ...
            requeue: boolean
                ...
                RULE:
                    ...
                    later stage.
        """
        return self.send_method(spec.Basic.Reject, argsig, (delivery_tag, requeue))
