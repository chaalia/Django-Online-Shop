...
from .text import cairo, dlopen, ffi, get_font_features, gobject, pango, pangocairo
...
if pango.pango_version() < 13800:
    ...
    def _check_font_configuration(font_config, warn=False):
        ...
        has_native_mode = sys.platform.startswith("win") or sys.platform.startswith(
            "darwin"
        )
        ...
        if config_file == ffi.NULL:
            ...
            if warn:
                warnings.warn("FontConfig: No fonts configured. " "Expect ugly output.")
            return True
        ...
        def __init__(self):
            ...
            if _check_font_configuration(self._fontconfig_config):
                ...
                pangoft2.pango_fc_font_map_set_config(
                    ffi.cast("PangoFcFontMap *", self.font_map), self._fontconfig_config
                )
            ...
            if sys.platform.startswith("win"):
                self._tempdir = os.path.join(tempfile.gettempdir(), "weasyprint")
                try:
                ...
                if font_type in ("external", "local"):
                    ...
                    if font_type == "local":
                        ...
                        if font_name.lower() in (family.lower(), postscript.lower()):
                            ...
                            path = ffi.string(filename[0]).decode(FILESYSTEM_ENCODING)
                    ...
                    try:
                        ...
                        LOGGER.debug('Failed to load font at "%s" (%s)', url, exc)
                        continue
                    font_features = {
                    ...
                    if "font_feature_settings" in rule_descriptors:
                        font_features["font_feature_settings"] = rule_descriptors[
                            "font_feature_settings"
                        ]
                    features_string = ""
                    for key, value in get_font_features(**font_features).items():
                        features_string += "<string>{} {}</string>".format(key, value)
