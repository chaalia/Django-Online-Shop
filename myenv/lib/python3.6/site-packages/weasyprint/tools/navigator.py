  ...
  value="'''
    ...
    if url:
        for width, height, data_url, links, anchors in get_pages(html):
            write(
                '<section style="width: {}px; height: {}px">\n'
                '  <img src="{}">\n'.format(width, height, data_url)
            )
            for link_type, target, (pos_x, pos_y, width, height) in links:
                href = "#" + target if link_type == "internal" else "/view/" + target
                write(
                    '  <a style="left: {}px; top: {}px; '
                    'width: {}px; height: {}px" href="{}"></a>\n'.format(
            ...
            for anchor_name, (pos_x, pos_y) in anchors.items():
                ...
                # the address bar.
                write(
                    '  <a style="left: {}px; top: {}px;" name="{}"></a>\n'.format(
                        pos_x, pos_y - 60, anchor_name
                    )
                )
            write("</section>\n")
    else:
...
def app(environ, start_response):
    def make_response(
        body, status="200 OK", headers=(), content_type="text/html; charset=UTF-8"
    ):
        start_response(
            status,
