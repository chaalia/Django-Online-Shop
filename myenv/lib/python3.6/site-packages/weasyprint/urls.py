...
def url_is_absolute(url):
    return bool(
        (UNICODE_SCHEME_RE if isinstance(url, str) else BYTES_SCHEME_RE).match(url)
    )
...
def url_join(base_url, url, allow_relative, context, context_args):
    ...
    if url_is_absolute(url):
        ...
        LOGGER.error(
            "Relative URI reference without a base URI: " + context, *context_args
        )
        return None
...
def fetch(url_fetcher, url):
    ...
    try:
        ...
        raise URLFetchingError("{}: {}".format(type(exc).__name__, str(exc)))
