...
def build_formatting_structure(
    element_tree, style_for, get_image_from_uri, base_url, target_collector
):
    ...
    if box_list:
        ...
        box, = element_to_box(
            element_tree, root_style_for, get_image_from_uri, base_url, target_collector
        )
...
def make_box(element_tag, style, content, element):
    box = BOX_TYPE_FROM_DISPLAY[style["display"]](element_tag, style, element, content)
    return box


def element_to_box(
    element, style_for, get_image_from_uri, base_url, target_collector, state=None
):
    ...
    if style["display"] == "list-item":
        marker_boxes = list(
            marker_to_box(
                element, state, style, style_for, get_image_from_uri, target_collector
            )
        )
    ...
    children.extend(
        before_after_to_box(
            element, "before", state, style_for, get_image_from_uri, target_collector
        )
    )
    ...
    children.extend(
        before_after_to_box(
            element, "after", state, style_for, get_image_from_uri, target_collector
        )
    )
...
def before_after_to_box(
    element, pseudo_type, state, style_for, get_image_from_uri, target_collector
):
    ...
    box = make_box("{}::{}".format(element.tag, pseudo_type), style, [], element)
...
def marker_to_box(
    element, state, parent_style, style_for, get_image_from_uri, target_collector
):
...
def _collect_missing_target_counter(
    counter_name, lookup_counter_values, anchor_name, missing_target_counters
):
    ...
    for type_, value in content_list:
        if type_ == "string":
            ...
            if need_collect_missing:
                _collect_missing_counter(counter_name, counter_values, missing_counters)
            ...
            if lookup_target.state == "up-to-date":
                ...
                texts = []
                break
        elif type_ == "quote" and quote_depth is not None and quote_style is not None:
...
def compute_string_set(
    element, box, string_name, content_list, counter_values, target_collector
):
    ...
    def parse_again(mixin_pagebased_counters=None):
        ...
        compute_string_set(
            element, box, string_name, content_list, local_counters, target_collector
        )
...
def set_content_lists(element, box, style, counter_values, target_collector):
    ...
    if style["bookmark_label"] == "none":
        ...
        compute_bookmark_label(
            element, box, style["bookmark_label"], counter_values, target_collector
        )
...
def table_boxes_children(box, children):
    ...
    if isinstance(box, boxes.TableColumnBox):  # rule 1.1
        ...
        if not children:
            children = [
                boxes.TableColumnBox.anonymous_from(box, []) for _i in range(box.span)
            ]
...
def collapse_table_borders(table, grid_width, grid_height):
    ...
    vertical_borders = [
        [weak_null_border for x in range(grid_width + 1)] for y in range(grid_height)
    ]
    horizontal_borders = [
        [weak_null_border for x in range(grid_width)] for y in range(grid_height + 1)
    ]
    ...
    for row_group in table.children:
        for row in row_group.children:
            for cell in row.children:
                ...
                set_borders(
                    cell, x=cell.grid_x, y=grid_y, w=cell.colspan, h=cell.rowspan
                )
            grid_y += 1
    ...
    for column_group in table.column_groups:
        set_borders(
            column_group, x=column_group.grid_x, y=0, w=column_group.span, h=grid_height
        )
    ...
    def max_vertical_width(x, y, h):
        return max(
            width
            for grid_row in vertical_borders[y : y + h]
        ...
        for row in row_group.children:
            ...
            for cell in row.children:
                set_transparent_border(
                    cell,
                    "top",
                    max_horizontal_width(x=cell.grid_x, y=grid_y, w=cell.colspan),
                )
                set_transparent_border(
                    cell,
                    "bottom",
                ...
                set_transparent_border(
                    cell,
                    "left",
                    max_vertical_width(x=cell.grid_x, y=grid_y, h=cell.rowspan),
                )
                set_transparent_border(
                    cell,
                    "right",
    ...
    set_transparent_border(table, "top", max_horizontal_width(x=0, y=0, w=grid_width))
    set_transparent_border(
        table, "bottom", max_horizontal_width(x=0, y=grid_height, w=grid_width)
    )
    ...
    set_transparent_border(table, "left", max_vertical_width(x=0, y=0, h=1))
    set_transparent_border(table, "right", max_vertical_width(x=grid_width, y=0, h=1))
        ...
        for child in children:
            ...
            if isinstance(child, boxes.InlineLevelBox):
                ...
                if isinstance(child, boxes.ParentBox):
                    anonymous = boxes.BlockBox.anonymous_from(box, child.children)
...
def _inner_block_in_inline(box, skip_stack=None):
    ...
    for i, child in enumerate(box.children[skip:]):
        ...
        if block_level_box is not None:
            ...
            box = box.copy_with_children(new_children, is_start=is_start, is_end=False)
            break
    else:
        if changed or skip:
            box = box.copy_with_children(new_children, is_start=is_start, is_end=True)
