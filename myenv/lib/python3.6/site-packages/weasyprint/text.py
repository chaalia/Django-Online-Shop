...
gobject = dlopen(
    ffi, "gobject-2.0", "libgobject-2.0-0", "libgobject-2.0.so", "libgobject-2.0.dylib"
)
pango = dlopen(
    ffi, "pango-1.0", "libpango-1.0-0", "libpango-1.0.so", "libpango-1.0.dylib"
)
pangocairo = dlopen(
    ffi,
    "pangocairo-1.0",
...
def get_size(line, style):
    ...
    width, height = (
        units_to_double(logical_extents.width),
...
class Layout(object):
    ...
    def setup(self, context, font_size, style):
        ...
        self.layout = ffi.gc(
            pangocairo.pango_cairo_create_layout(
                ffi.cast("cairo_t *", CAIRO_DUMMY_CONTEXT[hinting]._pointer)
            ),
    ...
    def get_first_line(self):
        layout_iter = ffi.gc(
            pango.pango_layout_get_iter(self.layout), pango.pango_layout_iter_free
        )
        first_line = pango.pango_layout_iter_get_line_readonly(layout_iter)
        if pango.pango_layout_iter_next_line(layout_iter):
            second_line = pango.pango_layout_iter_get_line_readonly(layout_iter)
...
def split_first_line(
    text, style, context, max_width, justification_spacing, minimum=False
):
    ...
    if max_width is not None and max_width != float("inf") and style["font_size"]:
        ...
        if expected_length < len(text):
            ...
            layout = create_layout(
                text[:expected_length], style, context, max_width, justification_spacing
            )
            ...
            if index is None:
                ...
                layout = None
    if layout is None:
        layout = create_layout(text, style, context, max_width, justification_spacing)
        first_line, index = layout.get_first_line()
    resume_at = index
        ...
        if next_word_boundaries:
            ...
            if stop_word - start_word >= total:
                ...
                if style["hyphenate_limit_zone"].unit == "%":
                    limit_zone = max_width * style["hyphenate_limit_zone"].value / 100.0
                else:
