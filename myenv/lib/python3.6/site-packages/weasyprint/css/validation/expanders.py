...
def expand_four_sides(base_url, name, tokens):
    ...
    if len(tokens) == 1:
        ...
        raise InvalidValues("Expected 1 to 4 token components got %i" % len(tokens))
...
def border_radius(base_url, name, tokens):
    ...
    for values in horizontal, vertical:
        ...
        if len(values) == 1:
            ...
            raise InvalidValues("Expected 1 to 4 token components got %i" % len(values))
    corners = ("top-left", "top-right", "bottom-right", "bottom-left")
    for corner, tokens in zip(corners, zip(horizontal, vertical)):
        new_name = "border-%s-radius" % corner
        # validate_non_shorthand returns [(name, value)], we want
        # to yield (name, value)
        result, = validate_non_shorthand(base_url, new_name, tokens, required=True)
        yield result
...
def expand_background(base_url, name, tokens):
    ...
    results = {k: [v] for k, v in last_layer.items()}
