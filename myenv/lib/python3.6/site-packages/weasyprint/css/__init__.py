...
PSEUDO_ELEMENTS = (None, "before", "after", "marker", "first-line", "first-letter")
    ...
    def __call__(self, element, pseudo_type=None):
        ...
        if style:
            if "table" in style["display"]:
                if (
                    style["display"] in ("table", "inline-table")
                ...
                if (
                    ...
                    and style["display"] != "table-caption"
                ):
...
def find_stylesheets(
    wrapper_element, device_media_type, url_fetcher, base_url, font_config, page_rules
):
        ...
        if element.tag == "style":
            ...
            if href is not None:
                try:
                    ...
                    LOGGER.error("Failed to load stylesheet at %s : %s", href, exc)
        ...
        if element.tag == "body":
            ...
            for part, position in (
                ...
                for prop in ("margin%s" % part, "%smargin" % position):
                    if element.get(prop):
                        style_attribute = "margin-{}:{}px".format(
            ...
            if element.get("cellspacing"):
                yield specificity, check_style_attribute(
                    element, "border-spacing:%spx" % element.get("cellspacing")
                )
                ...
                for subelement in element.iter():
                    if subelement.tag in ("td", "th"):
                        yield specificity, check_style_attribute(
                            ...
                            "padding-top:%s;padding-bottom:%s;" % (4 * (cellpadding,)),
                        )
            if element.get("hspace"):
                ...
                if hspace.isdigit():
                    hspace += "px"
                yield specificity, check_style_attribute(
                    element, "margin-left:{};margin-right:{}".format(hspace, hspace)
                )
            if element.get("vspace"):
                ...
                if vspace.isdigit():
                    vspace += "px"
                yield specificity, check_style_attribute(
                    element, "margin-top:{};margin-bottom:{}".format(vspace, vspace)
                )
            ...
            if element.get("bordercolor"):
                style_attribute = "border-color:%s" % (element.get("bordercolor"))
                yield specificity, check_style_attribute(element, style_attribute)
                ...
                if tokens[0].type == "ident":
                    ...
                    if group is not None:
                        group = [
                            token
                            for token in group
                            if token.type not in ("comment", "whitespacespace")
                        ]
                        if len(group) != 1:
                            return None
                        group, = group
        ...
        if rule.type == "qualified-rule":
            ...
            if url is not None:
                try:
                    ...
                    LOGGER.error("Failed to load stylesheet at %s : %s", url, exc)
            ...
            for page_type in data:
                ...
                for margin_rule in content:
                    ...
                    if declarations:
                        selector_list = [
                            (specificity, "@" + margin_rule.lower_at_keyword, page_type)
                        ]
