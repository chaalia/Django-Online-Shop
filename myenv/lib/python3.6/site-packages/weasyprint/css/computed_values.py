...
from .properties import INHERITED, INITIAL_NOT_COMPUTED, INITIAL_VALUES, Dimension
from .utils import (
    ANGLE_TO_RADIANS,
    LENGTH_UNITS,
    LENGTHS_TO_PIXELS,
...
BORDER_WIDTH_KEYWORDS = {"thin": 1, "medium": 3, "thick": 5}
...
def _resolve_var(computed, variable_name, default):
    ...
    while (
        computed_value
        and isinstance(computed_value, tuple)
...
def background_image(computer, name, values):
    ...
    for type_, value in values:
        ...
        if type_ == "radial-gradient":
            value.center, = compute_position(computer, name, (value.center,))
            if value.size_type == "explicit":
                value.size = length_or_percentage_tuple(computer, name, value.size)
    return values
...
def compute_position(computer, name, values):
    ...
    return tuple(
        (
            origin_x,
            length(computer, name, pos_x),
            origin_y,
            length(computer, name, pos_y),
        )
...
def length_tuple(computer, name, values):
    ...
    return tuple(length(computer, name, value, pixels_only=True) for value in values)
...
def length(computer, name, value, font_size=None, pixels_only=False):
    ...
    if unit == "px":
        ...
        if unit == "ex":
            ...
            layout = text.Layout(
                context=None, font_size=font_size, style=computer["computed"]
            )
            layout.set_text("0")
...
def background_size(computer, name, values):
    ...
    return tuple(
        ...
        else length_or_percentage_tuple(computer, name, value)
        for value in values
    )
...
def _content_list(computer, values):
    ...
    for value in values:
        ...
        if computed_value is None:
            LOGGER.warning(
                "Unable to compute {}'s value for content: {}".format(
...
def string_set(computer, name, values):
    ...
    return tuple(
        (string_set[0], _content_list(computer, string_set[1])) for string_set in values
    )
...
def display(computer, name, value):
    ...
    if (
        position in ("absolute", "fixed")
        or float_ != "none"
        or computer["is_root_element"]
    ):
...
def font_size(computer, name, value):
    ...
    if value == "larger":
        ...
        return value.value * parent_font_size / 100.0
    else:
        return length(
            computer, name, value, pixels_only=True, font_size=parent_font_size
        )
...
def link(computer, name, values):
    ...
    if values == "none":
        ...
        type_, value = values
        if type_ == "attr()":
            return get_link_attribute(computer["element"], value, computer["base_url"])
        else:
            return values
...
def vertical_align(computer, name, value):
    ...
    if value in ("baseline", "middle", "text-top", "text-bottom", "top", "bottom"):
        ...
        return height * value.value / 100.0
    else:
        return length(computer, name, value, pixels_only=True)
