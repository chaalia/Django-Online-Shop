...
def _gather_links_and_bookmarks(box, bookmarks, links, anchors, matrix):
    ...
    if has_bookmark or has_link or has_anchor:
        ...
        if has_bookmark:
            bookmarks.append((bookmark_level, bookmark_label, (pos_x, pos_y), state))
        if has_anchor:
    ...
    def paint(self, cairo_context, left_x=0, top_y=0, scale=1, clip=False):
        ...
        with stacked(cairo_context):
            ...
            if clip:
                ...
                height = self.height
                if self._enable_hinting:
                    width, height = cairo_context.user_to_device_distance(width, height)
                    # Hint in device space
                    width = int(math.ceil(width))
                    height = int(math.ceil(height))
                    width, height = cairo_context.device_to_user_distance(width, height)
...
class Document(object):
    ...
    def _build_layout_context(
        ...
        for css in stylesheets or []:
            if not hasattr(css, "matcher"):
                css = CSS(
                    guess=css, media_type=html.media_type, font_config=font_config
                )
    ...
    def copy(self, pages="all"):
        ...
        if pages == "all":
            ...
            pages = list(pages)
        return type(self)(pages, self.metadata, self.url_fetcher, self._font_config)
    ...
    def write_pdf(self, target=None, zoom=1, attachments=None):
        ...
        for page, links_and_anchors in zip(self.pages, paged_links_and_anchors):
            links, anchors = links_and_anchors
            surface.set_size(
                math.floor(
                    scale * (page.width + page.bleed["left"] + page.bleed["right"])
                ),
                math.floor(
                    scale * (page.height + page.bleed["top"] + page.bleed["bottom"])
                ),
            )
            with stacked(context):
                context.translate(page.bleed["left"] * scale, page.bleed["top"] * scale)
