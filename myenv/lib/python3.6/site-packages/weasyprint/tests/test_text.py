...
def make_text(text, width=None, **style):
    ...
    return split_first_line(
        text, new_style, context=None, max_width=width, justification_spacing=0
    )


@assert_no_logs
def test_line_content():
    for width, remaining in [(100, "text for test"), (45, "is a text for test")]:
...
def test_line_breaking():
    ...
    assert string.encode("utf-8")[resume_at:].decode("utf-8") == ("is a text for test")

    _, _, resume_at, _, _, _ = make_text(
        string, 100, font_family=SANS_FONTS.split(","), font_size=19
    )
    assert string.encode("utf-8")[resume_at:].decode("utf-8") == ("text for test")
...
def test_line_breaking_rtl():
    ...
    _, _, resume_at, _, _, _ = make_text(string, 90, font_size=100)
    assert string.encode("utf-8")[resume_at:].decode("utf-8") == ("ايبسوم دولا")
...
def test_hyphenate_manual_1():
    for i in range(1, len("hyphénation")):
        for hyphenate_character in ("!", "ù ù"):
            ...
            full_text = "".join(child.text for line in lines for child in line.children)
...
@assert_no_logs
@pytest.mark.parametrize(
    "wrap, text, test, full_text",
    (
        ("break-word", "aaaaaaaa", lambda a: a > 1, "aaaaaaaa"),
        ("normal", "aaaaaaaa", lambda a: a == 1, "aaaaaaaa"),
        ("break-word", "hyphenations", lambda a: a > 3, "hy\u2010phen\u2010ations"),
        (
            "break-word",
            "A splitted word.  An hyphenated word.",
            lambda a: a > 8,
            "Asplittedword.Anhy\u2010phen\u2010atedword.",
        ),
    ),
)
