...
def test_table_column_width_1():
    ...
    assert logs[0].startswith(
        "WARNING: This table row has more columns than " "the table, ignored 1 cell"
    )
    html, = page.children
    body, = html.children
...
def test_table_row_height_1():
    ...
    assert [[cell.height for cell in row.children] for row in row_group.children] == [
        [420, 60, 40, 20, 20, 20],
        [0, 10],
        [],
        [0],
    ...
    assert [
        [cell.border_height() for cell in row.children] for row in row_group.children
    ] == [[420, 80, 80, 80, 80, 80], [30, 30], [], [10]]
    ...
    assert [
        [cell.padding_top for cell in row.children] for row in row_group.children
    ] == [[0, 20, 0, 0, 30, 60], [15, 5], [], [10]]
    assert [
        [cell.padding_bottom for cell in row.children] for row in row_group.children
    ] == [[0, 0, 40, 60, 30, 0], [15, 15], [], [0]]
    assert [
        [cell.position_y for cell in row.children] for row in row_group.children
    ] == [[103, 103, 103, 103, 103, 103], [283, 283], [], [513]]
...
@assert_no_logs
@pytest.mark.parametrize(
    "vertical_align, table_position_y", (("top", 8), ("bottom", 8), ("baseline", 10))
)
...
@assert_no_logs
@pytest.mark.parametrize(
    "rows_expected, thead, tfoot, content",
    (
