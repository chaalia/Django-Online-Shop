...
def expand_to_dict(css, expected_error=None):
    ...
    return {
        name: value for name, value, _priority in declarations if value != "initial"
    }
...
def test_not_print():
    assert (
        expand_to_dict("volume: 42", "the property does not apply for the print media")
        == {}
    )
...
def test_spacing_warning():
    assert (
        expand_to_dict("letter_spacing: normal", "did you mean letter-spacing?") == {}
    )
...
@pytest.mark.parametrize(
    "rule",
    ("size: A3 landscape A3", "size: A9", "size: foo", "size: foo bar", "size: 20%"),
)
...
def test_expand_four_sides_warning():
    assert (
        expand_to_dict("padding: 1 2 3 4 5", "Expected 1 to 4 token components got 5")
        == {}
    )
...
@pytest.mark.parametrize(
    ...
    (
        ...
        (
            ...
            {"border_top_width": (3, "px"), "border_top_style": "dotted"},
        ),
        (
            "border-top: 3px red",
            {"border_top_width": (3, "px"), "border_top_color": (1, 0, 0, 1)},  # red
        ),
...
@pytest.mark.parametrize(
    ...
    (
        ...
        ("list-style: square", {"list_style_type": "square"}),
        (
            "list-style: circle inside",
            {"list_style_position": "inside", "list_style_type": "circle"},
        ),
        (
...
def test_expand_list_style_warning():
    assert_invalid(
        "list-style: circle disc", "got multiple type values in a list-style shorthand"
    )


@assert_no_logs
@pytest.mark.parametrize(
    "rule", ("list-style: none inside none none", "list-style: red")
)
...
def test_expand_background():
    ...
    assert_background(
        "top right", background_position=[("left", (100, "%"), "top", (0, "%"))]
    )
    assert_background(
        "top right 20px", background_position=[("right", (20, "px"), "top", (0, "%"))]
    )
    assert_background(
        "top 1% right 20px",
    ...
    assert_background(
        ...
        background_image=[("url", "http://weasyprint.org/foo/bar"), ("none", None)],
        background_position=[
            ("left", (50, "%"), "top", (50, "%")),
            ("left", (0, "%"), "top", (0, "%")),
        ],
...
def test_expand_background_position():
    ...
    for css_x, val_x in [
        ...
        for css_y, val_y in [
            ...
            # Two tokens:
            position("{} {}".format(css_x, css_y), "left", val_x, "top", val_y)
        # One token:
...
def test_string_set():
    ...
    assert expand_to_dict('string-set: test1 "string", test2 "string"') == {
        "string_set": (
            ("test1", (("string", "string"),)),
            ("test2", (("string", "string"),)),
        )
    ...
    assert expand_to_dict("string-set: test counter(count, upper-roman)") == {
    ...
    assert expand_to_dict('string-set: test counters(count, ".", upper-roman)') == {
        "string_set": (("test", (("counters()", ("count", ".", "upper-roman")),)),)
    }
    assert expand_to_dict(
    ...
    def gradient(css, direction, colors=[blue], stop_positions=[None]):
        for repeating, prefix in ((False, ""), (True, "repeating-")):
            expanded = expand_to_dict(
                "background-image: {}linear-gradient({})".format(prefix, css)
            )
...
def test_radial_gradient():
    ...
    def gradient(
        ...
        for repeating, prefix in ((False, ""), (True, "repeating-")):
            expanded = expand_to_dict(
                "background-image: {}radial-gradient({})".format(prefix, css)
            )
            [(_, [(type_, image)])] = expanded.items()
    ...
    gradient(
        "10px 50px ellipse, blue", "ellipse", ("explicit", ((10, "px"), (50, "px")))
    )
    gradient(
        "ellipse 10px 50px, blue", "ellipse", ("explicit", ((10, "px"), (50, "px")))
    )
...
@pytest.mark.parametrize(
    "rule, result",
    (
        ("flex: auto", {"flex_grow": 1, "flex_shrink": 1, "flex_basis": "auto"}),
        ("flex: none", {"flex_grow": 0, "flex_shrink": 0, "flex_basis": "auto"}),
        ("flex: 10", {"flex_grow": 10, "flex_shrink": 1, "flex_basis": ZERO_PIXELS}),
        ("flex: 2 2", {"flex_grow": 2, "flex_shrink": 2, "flex_basis": ZERO_PIXELS}),
        (
            "flex: 2 2 1px",
            {"flex_grow": 2, "flex_shrink": 2, "flex_basis": Dimension(1, "px")},
        ),
        ("flex: 2 2 auto", {"flex_grow": 2, "flex_shrink": 2, "flex_basis": "auto"}),
