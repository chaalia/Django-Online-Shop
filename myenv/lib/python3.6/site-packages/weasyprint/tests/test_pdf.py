...
@assert_no_logs
@pytest.mark.parametrize("width, height", ((100, 100), (200, 10), (3.14, 987654321)))
...
def test_page_size_zoom(zoom):
    pdf_bytes = FakeHTML(string="<style>@page{size:3in 4in").write_pdf(zoom=zoom)
    assert (
        "/MediaBox [ 0 0 {} {} ]".format(int(216 * zoom), int(288 * zoom)).encode(
            "ascii"
        )
        in pdf_bytes
    )
...
def test_bookmarks_3():
    ...
    assert o1.get_value("Title", "(.*)") == b"<FEFF006100A0006E0062007300702026>"
...
def test_bookmarks_7():
    ...
    o1 = outlines.get_indirect_dict("First", pdf_file)
    assert o1.get_value("Title", "(.*)") == b"(a)"
    assert float(o1.get_value("Dest", "\\[(.+?)\\]").strip().split()[-2]) == round(
        y * 1.5
    )
...
def test_links():
    ...
    assert_rect_almost_equal(links[0].get_value("Rect", "(.*)"), (0, TOP - 20, 30, TOP))

    # The image itself: 30*30pt
    assert links[1].get_value("URI", "(.*)") == b"(http://weasyprint.org)"
    assert links[1].get_value("S", "(.*)") == b"/URI"
    assert_rect_almost_equal(links[1].get_value("Rect", "(.*)"), (0, TOP - 30, 30, TOP))
    ...
    assert_rect_almost_equal(
        links[2].get_value("Rect", "(.*)"), (10, TOP - 100 - 20, 10 + 32, TOP - 100)
    )
    ...
    # assert [round(float(value)) for value in dest[-3:]] == […]
    assert_rect_almost_equal(
        links[3].get_value("Rect", "(.*)"), (10, TOP - 100 - 32, 10 + 32, TOP - 100)
    )
    ...
    assert dest == [b"(hello)"]
    names = (
        pdf_file.catalog.get_indirect_dict("Names", pdf_file)
        .get_indirect_dict("Dests", pdf_file)
        .byte_string
    ).decode("ascii")
    assert_rect_almost_equal(
        re.search("\\(hello\\) \\[\\d+ \\d+ R /XYZ (\\d+ \\d+ \\d+)]", names).group(1),
        (0, TOP - 200, 0),
    )
    assert_rect_almost_equal(
    ...
    assert link.get_value("URI", "(.*)") == b"(http://weasyprint.org/foo/lipsum)"
    assert link.get_value("S", "(.*)") == b"/URI"
    assert_rect_almost_equal(link.get_value("Rect", "(.*)"), (0, TOP, RIGHT, TOP))
...
def test_relative_links():
    ...
    fileobj = io.BytesIO()
    FakeHTML(
        string='<a href="../lipsum" style="display: block"></a>a', base_url=None
    ).write_pdf(target=fileobj)
    pdf_file = pdf.PDFFile(fileobj)
    ...
    assert_rect_almost_equal(annots.get_value("Rect", "(.*)"), (0, TOP, RIGHT, TOP))
...
def test_relative_links_missing_base():
    ...
    with capture_logs() as logs:
        FakeHTML(
            string='<div style="-weasy-link: url(../lipsum)">', base_url=None
        ).write_pdf(target=fileobj)
    pdf_file = pdf.PDFFile(fileobj)
    with pytest.raises(AttributeError):
        pdf_file.pages[0].get_indirect_dict_array("Annots", pdf_file)
...
def test_relative_links_internal():
    ...
    assert_rect_almost_equal(
        re.search("\\(lipsum\\) \\[\\d+ \\d+ R /XYZ (\\d+ \\d+ \\d+)]", names).group(1),
        (0, TOP, 0),
    )
    assert_rect_almost_equal(annots.get_value("Rect", "(.*)"), (0, TOP, RIGHT, TOP))
...
def test_embed_gif():
    assert (
        b"/Filter /DCTDecode"
        not in FakeHTML(
            base_url=resource_filename("dummy.html"), string='<img src="pattern.gif">'
        ).write_pdf()
    )
...
def test_embed_jpeg():
    ...
    assert (
        b"/Filter /DCTDecode"
        in FakeHTML(
            base_url=resource_filename("dummy.html"), string='<img src="blue.jpg">'
        ).write_pdf()
    )
...
def test_document_info():
    ...
    assert info.get_value("Subject", "(.*)") == (b"<FEFF0042006C0061006820260020>")
...
def test_embedded_files_attachments(tmpdir):
    ...
    FakeHTML(
        string="""
          ...
          <h2>Heading 2</h2>
        """.format(
            absolute_url, os.path.basename(relative_tmp_file)
        ),
    ...
    assert (
        "<{}>".format(hashlib.md5(b"hi there").hexdigest()).encode("ascii") in pdf_bytes
    )
    ...
    assert os.path.basename(absolute_tmp_file).encode("utf-16-be") in pdf_bytes

    assert hashlib.md5(rdata).hexdigest().encode("ascii") in pdf_bytes
    assert os.path.basename(relative_tmp_file).encode("utf-16-be") in pdf_bytes

    assert hashlib.md5(b"oob attachment").hexdigest().encode("ascii") in pdf_bytes
    assert b"/Desc (\xfe\xff\x00H\x00e\x00l\x00l\x00o)" in pdf_bytes
    assert hashlib.md5(b"raw URL").hexdigest().encode("ascii") in pdf_bytes
    assert hashlib.md5(b"file like obj").hexdigest().encode("ascii") in pdf_bytes
...
def test_bleed(style, media, bleed, trim):
    ...
    pdf_bytes = fileobj.getvalue()
    assert "/MediaBox [ {} {} {} {} ]".format(*media).encode("ascii") in pdf_bytes
    assert "/BleedBox [ {} {} {} {} ]".format(*bleed).encode("ascii") in pdf_bytes
    assert "/TrimBox [ {} {} {} {} ]".format(*trim).encode("ascii") in pdf_bytes
