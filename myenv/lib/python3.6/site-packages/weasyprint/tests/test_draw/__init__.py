...
def assert_pixels(name, expected_width, expected_height, expected_pixels, html):
    ...
    assert_pixels_equal(name, expected_width, expected_height, pixels, expected_raw)


def assert_same_rendering(expected_width, expected_height, documents, tolerance=0):
    ...
    for name, pixels in pixels_list[1:]:
        assert_pixels_equal(
            name, expected_width, expected_height, reference, pixels, tolerance
        )
    ...
    for i, (name_1, pixels_1) in enumerate(pixels_list):
        for name_2, pixels_2 in pixels_list[i + 1 :]:
            if pixels_1 == pixels_2:  # pragma: no cover
                ...
                assert False, "{} and {} are the same".format(name_1, name_2)
...
def write_png(basename, pixels, width, height):  # pragma: no cover
    ...
    cairo.ImageSurface(
        cairo.FORMAT_ARGB32, width, height, data=bytearray(pixels), stride=width * 4
...
def html_to_pixels(name, expected_width, expected_height, html):
    ...
    pixels = document_to_pixels(document, name, expected_width, expected_height)
    ...
    if raw != expected_raw:  # pragma: no cover
        for i, (value, expected) in enumerate(zip(raw, expected_raw)):
            if abs(value - expected) > tolerance:
                ...
                expected_pixel = tuple(list(expected_raw[i : i + 4]))
                assert 0, "Pixel (%i, %i) in %s: expected rgba%s, got rgba%s" % (
                    x,
                    y,
                    name,
                    expected_pixel,
