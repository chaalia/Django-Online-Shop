...
def _test_resource(class_, basename, check, **kwargs):
    ...
    check(
        class_(
            string=content.decode(encoding),  # unicode
            base_url=relative_filename,
            **kwargs
        )
    )
    ...
    for page in pages:
        ...
        for i, link in enumerate(links):
            ...
            link = (
                link_type,
                target,
                (round(pos_x, 6), round(pos_y, 6), round(width, 6), round(height, 6)),
            )
            links[i] = link
        bookmarks = page.bookmarks
        for i, (level, label, (pos_x, pos_y), state) in enumerate(bookmarks):
            bookmarks[i] = (level, label, (round(pos_x, 6), round(pos_y, 6)), state)
...
def test_html_parsing():
    ...
    _test_resource(FakeHTML, "doc1_UTF-16BE.html", _check_doc1, encoding="UTF-16BE")
    ...
    string_with_meta = string.replace("<meta", '<base href="resources/"><meta')
...
def test_css_parsing():
    ...
    def check_css(css):
        ...
        assert h1_rule[4][1][1][0][1].endswith("weasyprint/tests/resources/pattern.png")
...
def test_python_render(tmpdir):
    ...
    assert (
        FakeHTML(string=html_string, base_url=base_url, media_type="screen").write_png(
    ...
    assert (
        FakeHTML(
            string="<style>{}</style>{}".format(css_string, html_string),
            base_url=base_url,
            media_type="screen",
        ).write_png()
        == rotated_png_bytes
...
def test_command_line_render(tmpdir):
    ...
    rotated_png_bytes = FakeHTML(
        string=combined, base_url="dummy.html", media_type="screen"
    ).write_png()
    empty_png_bytes = FakeHTML(string=b"<style>" + css + b"</style>").write_png()
    check_png_pattern(png_bytes)
    check_png_pattern(rotated_png_bytes, rotated=True)
...
@pytest.mark.parametrize(
                ...
                (
                    ...
                    [
                        ...
                        (
                            "Title 4",
                            (0, 0, 300),
                            [("Title 5", (0, 0, 400), [], "open")],
                            "open",
                        ),
                        ("Title 6", (1, 0, 100), [], "open"),
                    ],
                    "open",
                ),
                (
    ...
    """,
            ...
            [
                ("A", (0, 0, 0), [("B", (0, 0, 20), [], "open")], "open"),
                (
                    "C",
                    (0, 0, 40),
                    [("D", (0, 0, 60), [("E", (0, 0, 80), [], "open")], "open")],
                    "open",
                ),
            ],
            False,
    ...
    """,
        ...
        ("<h1>é", [[(1, "é", (0, 0), "open")]], [("é", (0, 0, 0), [], "open")], False),
        (
            """
...
def test_links():
    ...
    """,
        ...
        [
            ...
            ([("internal", "hello", (0, 0, 200, 30))], [("lipsum", 0, 0)]),
        ],
    )
    ...
    assert_links(
        ...
        [
            [
                (
                    "external",
                    "http://weasyprint.org/foo/lipsum/%C3%A9_%E9",
                    (5, 10, 190, 0),
                )
            ]
        ],
    ...
    assert_links(
        ...
        base_url=None,
        warnings=["ERROR: No anchor #missing for internal URI reference"],
    )
...
def test_url_fetcher():
    ...
    def fetcher(url):
        if url == "weasyprint-custom:foo/%C3%A9_%e9_pattern":
            ...
            return dict(
                string="body { background: url(é_%e9_pattern)", mime_type="text/css"
            )
        else:
    ...
    test(
        '<body><li style="list-style: inside '
    ...
    with http_server(
        {
            "/gzip": lambda env: (
                ...
                else (b"<html test=accept-encoding-header-fail>", [])
            ),
            "/deflate": lambda env: (
                (zlib.compress(b"<html test=ok>"), [("Content-Encoding", "deflate")])
                if "deflate" in env.get("HTTP_ACCEPT_ENCODING", "")
                else (b"<html test=accept-encoding-header-fail>", [])
            ),
        ...
        assert HTML(root_url + "/raw-deflate").etree_element.get("test") == "ok"
