...
TEST_UA_STYLESHEET = CSS(
    filename=os.path.join(os.path.dirname(__file__), "..", "css", "tests_ua.css")
)
...
def capture_logs():
    ...
    def emit(record):
        if record.name == "weasyprint.progress":
            return
        message = "{}: {}".format(record.levelname.upper(), record.getMessage())
...
def requires(library_name, expected_tuple):
    ...
    library_tuple = tuple(
        int(i) for i in (library_version[:2], library_version[2:4], library_version[4:])
    )
    return pytest.mark.skipif(
        library_tuple < expected_tuple,
        reason="Running {} {} but this test requires {}+".format(
            library_name, "%i.%i.%i" % library_tuple, "%i.%i.%i" % expected_tuple
        ),
    )
