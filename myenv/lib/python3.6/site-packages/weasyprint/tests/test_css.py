...
def test_page():
    ...
    page_type = PageType(side="left", first=True, blank=False, index=0, name="")
    ...
    page_type = PageType(side="right", first=True, blank=False, index=0, name="")
    set_page_type_computed_styles(page_type, document, style_for)
    style = style_for(page_type)
    assert style["margin_top"] == (5, "px")
    assert style["margin_left"] == (10, "px")
    ...
    page_type = PageType(side="left", first=False, blank=False, index=1, name="")
    set_page_type_computed_styles(page_type, document, style_for)
    style = style_for(page_type)
    ...
    page_type = PageType(side="right", first=False, blank=False, index=1, name="")
    set_page_type_computed_styles(page_type, document, style_for)
    style = style_for(page_type)
    assert style["margin_top"] == (10, "px")
    assert style["margin_left"] == (10, "px")
...
def test_units(value, width):
    document = FakeHTML(
      ...
      <body style="font: 10px ahem"><p style="margin-left: %s"></p>"""
        % value,
    )
    page, = document.render().pages
    html, = page._page_box.children
...
@assert_no_logs
@pytest.mark.parametrize(
    "parent_css, parent_size, child_css, child_size",
    (
        ("10px", 10, "10px", 10),
...
def test_font_size(parent_css, parent_size, child_css, child_size):
    ...
    style_for = get_all_computed_styles(
        ...
        user_stylesheets=[
            CSS(
                string="p{{font-size:{}}}span{{font-size:{}}}".format(
                    parent_css, child_css
                )
            )
        ],
    )
