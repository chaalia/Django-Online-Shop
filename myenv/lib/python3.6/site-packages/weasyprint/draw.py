...
def draw_page(page, context, enable_hinting):
    ...
    draw_background(context, page.canvas_background, enable_hinting, clip_box=False)
    ...
    with stacked(context):
        ...
        with stacked(context):
            ...
            if isinstance(box, boxes.InlineBox):
                draw_inline_level(context, stacking_context.page, box, enable_hinting)
                ...
                if isinstance(block, boxes.ReplacedBox):
                    ...
                    for child in block.children:
                        if isinstance(child, boxes.LineBox):
                            draw_inline_level(
                                context, stacking_context.page, child, enable_hinting
                            )
...
def rounded_box_path(context, radii):
    ...
    for i, (w, h, (rx, ry)) in enumerate(
        ...
        context.arc(
            (-1 if w else 1) * radius,
            (-1 if h else 1) * radius,
            radius,
            (2 + i) * pi / 2,
            (3 + i) * pi / 2,
        )
...
def draw_background(context, bg, enable_hinting, clip_box=True, bleed=None, marks=()):
...
def draw_background_image(context, layer, image_rendering):
    ...
    with stacked(context):
        if not layer.unbounded:
            context.rectangle(painting_x, painting_y, painting_width, painting_height)
            context.clip()
        # else: unrestricted, whole page box

        context.translate(positioning_x + position_x, positioning_y + position_y)
...
def draw_border(context, box, enable_hinting):
            ...
            for child in box.children[1:]:
                with stacked(context):
                    ...
                    draw_rect_border(
                        ...
                        styled_color(
                            box.style["column_rule_style"],
                            get_color(box.style, "column_rule_color"),
                            "left",
                        ),
                    )
    ...
    for side, width, color, style in zip(SIDES, widths, colors, styles):
        ...
        with stacked(context):
            ...
            draw_rounded_border(context, box, style, styled_color(style, color, side))
...
def clip_border_segment(
    ...
    if (
        ...
        and (
            ...
            hypot(*context.user_to_device(width, 0)) >= 1
            and hypot(*context.user_to_device(0, width)) >= 1
        )
    ):
            ...
            def draw_dots(dashes, line, way, x, y, px, py, chl):
                ...
                for i in range(0, dashes, 2):
                    ...
                    if side in ("top", "bottom"):
                        ...
                        context.line_to(main_offset, y + py + way * py * tan(angle2))
                        context.line_to(main_offset, y + py + way * py * tan(angle1))
...
def draw_outlines(context, box, enable_hinting):
    ...
    if box.style["visibility"] == "visible" and width and color.alpha:
        outline_box = (
            box.border_box_x() - width,
            box.border_box_y() - width,
            box.border_width() + 2 * width,
            box.border_height() + 2 * width,
        )
        for side in SIDES:
            with stacked(context):
                clip_border_segment(
...
def draw_collapsed_borders(context, table, enable_hinting):
    ...
    def half_max_width(border_list, yx_pairs, vertical=True):
        ...
        for y, x in yx_pairs:
            if (
                ...
                if vertical
                else (0 <= y <= grid_height and 0 <= x < grid_width)
            ):
    ...
    def add_vertical(x, y):
        ...
        pos_y1 = row_positions[y] - half_max_width(
            horizontal_borders, [(y, x - 1), (y, x)], vertical=False
        )
        pos_y2 = row_positions[y + 1] + half_max_width(
    ...
    def add_horizontal(x, y):
        ...
        pos_x1 = column_positions[x] - half_max_width(
            vertical_borders, [(y - 1, x), (y, x)]
        )
        pos_x2 = column_positions[x + 1] + half_max_width(
    ...
    for segment in segments:
        ...
        with stacked(context):
            clip_border_segment(
                context, enable_hinting, style, width, side, border_box, widths
            )
            draw_rect_border(
                context, border_box, widths, style, styled_color(style, color, side)
            )
...
def draw_inline_level(
    context, page, box, enable_hinting, offset_x=0, text_overflow="clip"
):
        ...
        if isinstance(box, (boxes.InlineBox, boxes.LineBox)):
            ...
            for child in box.children:
                ...
                if isinstance(child, boxes.TextBox):
                    draw_text(
                        context, child, enable_hinting, child_offset_x, text_overflow
                    )
                else:
                    draw_inline_level(
                        context,
...
def draw_text(context, textbox, enable_hinting, offset_x=0, text_overflow="clip"):
    ...
    if "overline" in values or "line-through" in values or "underline" in values:
...
def draw_wave(context, x, y, width, offset_x, radius):
    ...
    while width > 0:
        ...
        if up:
            ...
            context.arc_negative(center_x, y, radius, -alpha1, -alpha2)
...
def draw_text_decoration(
    context, textbox, offset_x, offset_y, thickness, enable_hinting, color
):
    ...
    with stacked(context):
        ...
        if style == "double":
            delta = 2 * thickness
            context.move_to(textbox.position_x, textbox.position_y + offset_y + delta)
            context.rel_line_to(textbox.width, 0)
