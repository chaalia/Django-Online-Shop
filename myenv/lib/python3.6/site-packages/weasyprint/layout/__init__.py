...
def layout_fixed_boxes(context, pages, containing_page):
    ...
    for page in pages:
        for box in page.fixed_boxes:
            ...
            absolute_boxes = []
            yield absolute_box_layout(context, box, containing_page, absolute_boxes)
            while absolute_boxes:
                new_absolute_boxes = []
                for box in absolute_boxes:
                    absolute_layout(context, box, containing_page, new_absolute_boxes)
...
def layout_document(html, root_box, context, max_loops=8):
    ...
    for loop in range(max_loops):
        if loop > 0:
            PROGRESS_LOGGER.info("Step 5 - Creating layout - Repagination #%i" % loop)
    ...
    for i, page in enumerate(pages):
        ...
        resume_at, next_page, right_page, page_state, remake_state = context.page_maker[
            i + 1
        ]
        page_counter_values = page_state[1]
        ...
        for child in page.descendants():
            ...
            if hasattr(child, "missing_link"):
                for (
                    (box, css_token),
                    item,
                ) in context.target_collector.counter_lookup_items.items():
    ...
    for i, page in enumerate(pages):
        ...
        page.children = (root,) + tuple(make_margin_boxes(context, page, state))
...
class LayoutContext(object):
    def __init__(
        ...
        self.running_elements = defaultdict(lambda: defaultdict(lambda: list()))
