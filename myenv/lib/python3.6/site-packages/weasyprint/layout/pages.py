...
class OrientedBox(object):
    ...
    def shrink_to_fit(self, available):
        self.inner = min(max(self.min_content_size, available), self.max_content_size)
...
def compute_variable_dimension(context, side_boxes, vertical, outer_sum):
    ...
    if box_b.box.is_generated:
        if box_b.inner == "auto":
            ...
            if outer_sum >= (box_b.outer_max_content_size + ac_max_content_size):
                ...
                if available > 0:
                    ...
                    weight_b = box_b.max_content_size - box_b.min_content_size
        ...
        if box_a.inner == box_c.inner == "auto":
            if outer_sum >= (
                ...
                if available > 0:
                    weight_a = box_a.max_content_size - box_a.min_content_size
                    weight_c = box_c.max_content_size - box_c.min_content_size
...
def make_margin_boxes(context, page, state):
    ...
    for prefix, vertical, containing_block, position_x, position_y in [
        ...
        side_boxes = [
            make_box("@{}-{}".format(prefix, suffix), containing_block)
...
def remake_page(index, context, root_box, html):
    ...
    page, resume_at, next_page = make_page(
        context, root_box, page_type, initial_resume_at, page_number, page_state
    )
    assert next_page
    if blank:
