...
def flex_layout(
    ...
    for child in children:
        ...
        child.hypothetical_main_size = max(
            getattr(child, "min_%s" % axis),
            min(child.flex_base_size, getattr(child, "max_%s" % axis)),
        )
    ...
    for i, child in enumerate(sorted(children, key=lambda item: item.style["order"])):
        if not child.is_flex_item:
            continue
    ...
    for line in flex_lines:
        # Step 6 - 9.7.1
        hypothetical_main_size = sum(child.hypothetical_main_size for i, child in line)
        ...
        for i, child in line:
            ...
            if (
                ...
                or (
                    flex_factor_type == "grow"
                    and child.flex_base_size > child.hypothetical_main_size
                )
                or (
                    flex_factor_type == "shrink"
                    and child.flex_base_size < child.hypothetical_main_size
    ...
    for line in flex_lines:
        ...
        for i, child in line:
            ...
            new_child, _, _, adjoining_margins, _ = blocks.block_level_layout_switch(
                context,
                child_copy,
                float("inf"),
                child_skip_stack,
            ...
            if cross == "height":
                ...
                child.margin_bottom += blocks.collapse_margin(adjoining_margins)
            else:
        ...
        for line in flex_lines:
            ...
            for i, child in line:
                ...
                if (
                    ...
                    and align_self == "baseline"
                    and child.margin_top != "auto"
                    and child.margin_bottom != "auto"
                ):
    ...
    for line in flex_lines:
        ...
        for i, child in line:
            ...
            position_axis += (
                child.margin_width() if axis == "width" else child.margin_height()
            )
                ...
                for i, child in line:
                    if child.is_flex_item:
                        ...
                        if box.style["align_content"] == "flex-end":
                            ...
                            setattr(
                                child, direction, current_value + extra_cross_size / 2
                            )
                            ...
                            setattr(
                                ...
                                direction,
                                current_value
                                + extra_cross_size / (len(flex_lines) + 1),
                            )
