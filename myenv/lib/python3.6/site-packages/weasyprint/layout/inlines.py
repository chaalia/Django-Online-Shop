...
def get_next_linebox(
    ...
    while 1:
        ...
        if (position_x, position_y) == (linebox.position_x, linebox.position_y):
    ...
    for waiting_float in waiting_floats:
        ...
        waiting_float = float_layout(
            context, waiting_float, containing_block, absolute_boxes, fixed_boxes
        )
...
def replaced_box_height(box):
    ...
    if box.height == "auto" and box.width == "auto" and intrinsic_height is not None:
        box.height = intrinsic_height
...
def split_inline_box(
    ...
    for i, child in enumerate(box.children[skip:]):
        ...
        if child.is_absolutely_positioned():
            ...
            non_floating_children = [
                child_
                for _, child_ in (children + waiting_children)
            ...
            if float_width > max_x - position_x or waiting_floats:
                ...
                for _, old_child in line_children:
                    ...
                    if (
                        ...
                        and box.style["direction"] == "ltr"
                    ) or (
                        child.style["float"] == "right"
                        and box.style["direction"] == "rtl"
                    ):
        ...
        new_child, resume_at, preserved, first, last, new_float_widths = split_inline_level(
            context,
            child,
            position_x,
            available_width,
        ...
        if new_child is None:
            ...
            trailing_whitespace = (
                isinstance(new_child, boxes.TextBox) and not new_child.text.strip()
            )
                ...
                if waiting_children:
                    ...
                    while waiting_children_copy:
                        ...
                        if child.is_in_normal_flow() and can_break_inside(child):
                            ...
                            child_new_child, child_resume_at, _, _, _, _ = split_inline_level(
                                context,
                                child,
                                child.position_x,
                                max_x,
                            ...
                            if initial_skip_stack is None:
                                ...
                                initial_index, current_skip_stack = initial_skip_stack
                            ...
                            while current_skip_stack and current_resume_at:
                                skip, current_skip_stack = current_skip_stack
                                resume, current_resume_at = current_resume_at
    ...
    new_box = box.copy_with_children(
        [box_child for index, box_child in children], is_start=is_start, is_end=is_end
    )
    if isinstance(box, boxes.LineBox):
        # We must reset line box width according to its new children
        in_flow_children = [
            box_child for box_child in new_box.children if box_child.is_in_normal_flow()
        ]
        if in_flow_children:
            new_box.width = (
    ...
    half_leading = (line_height - new_box.height) / 2.0
...
def split_text_box(context, box, available_width, skip):
    ...
    if length > 0:
        ...
        half_leading = (line_height - height) / 2.0
...
def line_box_verticality(box):
    ...
    subtrees_with_min_max = [
        ...
        for sub_max_y, sub_min_y in [
            ...
            else aligned_subtree_verticality(subtree, top_bottom_subtrees, baseline_y=0)
        ]
    ]
...
def inline_box_verticality(box, top_bottom_subtrees, baseline_y):
    ...
    for child in box.children:
        ...
        if vertical_align == "baseline":
            ...
            top = baseline_y - (one_ex + child.margin_height()) / 2.0
    ...
    if isinstance(box, boxes.TextBox):
        ...
        if nb_spaces > 0:
            layout = create_layout(
                box.text, box.style, context, float("inf"), box.justification_spacing
            )
            layout.deactivate()
...
def is_phantom_linebox(linebox):
    ...
    for child in linebox.children:
        if isinstance(child, boxes.InlineBox):
            ...
            for side in ("top", "right", "bottom", "left"):
                if (
                    getattr(child.style["margin_%s" % side], "value", None)
                    or child.style["border_%s_width" % side]
                    or child.style["padding_%s" % side].value
                ):
                    return False
