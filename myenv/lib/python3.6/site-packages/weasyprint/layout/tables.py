...
def table_layout(
    ...
    if table.style["border_collapse"] == "collapse":
        ...
        if horizontal_borders:
            table.border_top_width = (
                max(width for _, (_, width, _) in horizontal_borders[skipped_rows]) / 2
            )

    # Make this a sub-function so that many local variables like rows_x
    # don't need to be passed as parameters.
    def group_layout(group, position_y, max_position_y, page_is_empty, skip_stack):
        resume_at = None
        ...
        for i, row in enumerate(group.children[skip:]):
            ...
            for cell in row.children:
                ...
                if cell.colspan == 0:
                    ...
                    LOGGER.warning(
                        "This table row has more columns than "
            ...
            if ending_cells:  # in this row
                if row.height == "auto":
                    ...
                    row.height = max(
                        row.height, max(row_cell.height for row_cell in ending_cells)
                    )
        ...
        group = group.copy_with_children(
            new_group_children, is_start=is_group_start, is_end=resume_at is None
        )
    ...
    def body_groups_layout(skip_stack, position_y, max_position_y, page_is_empty):
    ...
    def all_groups_layout():
        ...
        for group in table.children[skip:]:
            if not group.is_header and not group.is_footer:
                avoid_breaks = group.style["break_inside"] in ("avoid", "avoid-page")
                break
    ...
    if (
        ...
        and (table.style["break_inside"] in ("avoid", "avoid-page"))
    ):
        table = None
        resume_at = None
    adjoining_margins = []
...
def auto_table_layout(context, box, containing_block):
    ...
    if table.style["border_collapse"] == "collapse":
        available_width -= table.border_left_width + table.border_right_width
...
def find_in_flow_baseline(box, last=False, baseline_types=(boxes.LineBox,)):
    ...
    if isinstance(box, boxes.ParentBox) and not isinstance(box, boxes.TableCaptionBox):
...
def distribute_excess_width(
    ...
    columns = [
        ...
        if not constrainedness[i + column_slice.start]
        and column_intrinsic_percentages[i + column_slice.start] == 0
        and column_max_content_widths[i + column_slice.start] > 0
    ]
    if columns:
    ...
    columns = [
        (i + column_slice.start, column)
        for i, column in enumerate(grid[column_slice])
        if constrainedness[i + column_slice.start]
        and column_intrinsic_percentages[i + column_slice.start] == 0
        and column_max_content_widths[i + column_slice.start] > 0
    ]
    if columns:
    ...
    if columns:
        ...
        percentage_width = sum(column_intrinsic_percentages[i] for i, column in columns)
        ...
        widths = [column_intrinsic_percentages[i] * ratio for i, column in columns]
        current_widths = [column_widths[i] for i, column in columns]
        # Allow to reduce the size of the columns to respect the percentage
        differences = [width[0] - width[1] for width in zip(widths, current_widths)]
        if sum(differences) > excess_width:
            differences = [
                difference / sum(differences) * excess_width
                for difference in differences
    ...
    columns = [
        ...
        if any(column)
        and column_intrinsic_percentages[i + column_slice.start] == 0
        and not any(max_content_width(context, cell) for cell in column if cell)
    ]
    if columns:
        for i in columns:
            column_widths[i] += excess_width / len(columns)
