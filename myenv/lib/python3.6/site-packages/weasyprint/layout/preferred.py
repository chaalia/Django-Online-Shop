...
def shrink_to_fit(context, box, available_width):
    ...
    *Warning:* both available_outer_width and the return value are
    ...
    """
    return min(
        max(min_content_width(context, box, outer=False), available_width),
...
def min_content_width(context, box, outer=True):
    ...
    if box.is_table_wrapper:
        ...
        box, (boxes.BlockContainerBox, boxes.TableColumnBox, boxes.FlexBox)
    ):
        ...
        raise TypeError("min-content width for %s not handled yet" % type(box).__name__)
...
def max_content_width(context, box, outer=True):
    ...
    if box.is_table_wrapper:
        ...
        raise TypeError("max-content width for %s not handled yet" % type(box).__name__)
...
def margin_width(box, width, left=True, right=True):
    ...
    if percentages < 100:
        return width / (1 - percentages / 100.0)
    else:
        ...
        if isinstance(child, boxes.InlineBox):
            ...
            if minimum and child_text == " ":
                ...
                while new_resume_at is not None:
                    ...
                    _, _, new_resume_at, width, _, _ = text.split_first_line(
                        child_text[resume_at:],
                        child.style,
                        context,
                        max_width,
...
def table_and_columns_preferred_widths(context, box, outer=True):
        ...
        for groups in (column_groups, columns):
            if groups[i]:
                min_content_widths[i] = max(
                    min_content_widths[i], min_content_width(context, groups[i])
                )
                max_content_widths[i] = max(
                    max_content_widths[i], max_content_width(context, groups[i])
                )
                intrinsic_percentages[i] = max(
                    intrinsic_percentages[i], _percentage_contribution(groups[i])
                )
        ...
        for i in range(grid_width):
            ...
            for j, cell in enumerate(zipped_grid[i]):
                ...
                if intrinsic_percentages[i] == 0:
                    diff = max(
                        0, _percentage_contribution(origin_cell) - baseline_percentage
                    )
    ...
    small_percentage_contributions = [
        max_content_widths[i] / (intrinsic_percentages[i] / 100.0)
        for i in range(grid_width)
