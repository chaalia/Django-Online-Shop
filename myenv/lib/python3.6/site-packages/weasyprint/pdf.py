    ...
    * The new !P conversion flags encodes a PDF string.
    ...
    def convert_field(self, value, conversion):
        if conversion == "P":
            ...
            return "({})".format(
                pdf_escape(("\ufeff" + value).encode("utf-16-be").decode("latin1"))
            )
        else:
...
class PDFDictionary(object):
    ...
    def __repr__(self):
        return self.__class__.__name__ + repr((self.object_number, self.byte_string))
...
class PDFFile(object):
    ...
    def overwrite_object(self, object_number, byte_string):
        ...
        """
        self.overwritten_objects_offsets[object_number] = self._write_object(
            object_number, byte_string
        )
    ...
    def write_new_object(self, byte_string):
        ...
        self.new_objects_offsets.append(self._write_object(object_number, byte_string))
...
def write_pdf_metadata(
    fileobj, scale, url_fetcher, attachments, attachment_links, pages
):
    ...
    embedded_files_id = _write_pdf_embedded_files(pdf, attachments, url_fetcher)
    ...
    for pdf_page, document_page, page_links in zip(pdf.pages, pages, attachment_links):

        # Add bleed box

        media_box = (
            pdf_page.get_value("MediaBox", "\\[(.+?)\\]").decode("ascii").strip()
        )
        left, top, right, bottom = (float(value) for value in media_box.split(" "))
        # Convert pixels into points
        bleed = {key: value * 0.75 for key, value in document_page.bleed.items()}
        ...
        if annotations:
            pdf.extend_dict(
                pdf_page,
                pdf_format(
                    "/Annots [{0}]", " ".join("{} 0 R".format(n) for n in annotations)
                ),
            )
