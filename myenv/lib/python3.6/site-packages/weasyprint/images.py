...
class ImageLoadingError(ValueError):
    ...
    def from_exception(cls, exception):
        ...
        value = str(exception)
        return cls("{}: {}".format(name, value) if value else name)
...
class RasterImage(object):
    ...
    def draw(self, context, concrete_width, concrete_height, image_rendering):
        ...
        context.scale(
            concrete_width / self._intrinsic_width,
            concrete_height / self._intrinsic_height,
        )
        context.set_source_surface(self.image_surface)
        context.get_source().set_filter(IMAGE_RENDERING_TO_FILTER[image_rendering])
        context.paint()
...
class SVGImage(object):
    def __init__(self, svg_data, base_url, url_fetcher):
        ...
        self._base_url = base_url if not base_url.lower().startswith("data:") else None
    ...
    def get_intrinsic_size(self, _image_resolution, font_size):
        ...
        self._width = cairosvg.surface.size(fake_surface, self._tree.get("width"))
        self._height = cairosvg.surface.size(fake_surface, self._tree.get("height"))
    ...
    def draw(self, context, concrete_width, concrete_height, _image_rendering):
        try:
            ...
            LOGGER.error("Failed to draw an SVG image at %s : %s", self._base_url, e)
            ...
            if mime_type == "image/svg+xml":
                ...
                if not image:
                    ...
                    try:
                        ...
                        try:
                            surface, format_name = pixbuf.decode_to_image_surface(
                                string
                            )
...
def process_color_stops(gradient_line_size, positions):
    ...
    """
    positions = [percentage(position, gradient_line_size) for position in positions]
...
def gradient_average_color(colors, positions):
    ...
    # Un-premultiply:
    return (
        (result_r / result_a, result_g / result_a, result_b / result_a, result_a)
        if result_a != 0
        else (0, 0, 0, 0)
    )
...
class Gradient(object):
    ...
    def draw(self, context, concrete_width, concrete_height, _image_rendering):
        ...
        pattern.set_extend(
            cairocffi.EXTEND_REPEAT if self.repeating else cairocffi.EXTEND_PAD
        )
