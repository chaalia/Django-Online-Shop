...
def ipn(request):
    ...
    if data is not None:
        ...
        if form.is_valid():
            ...
            formatted_form_errors = [
                "{}: {}".format(k, ", ".join(v)) for k, v in form.errors.items()
            ]
            flag = "Invalid form. ({})".format(", ".join(formatted_form_errors))
    ...
    if encoding_missing:
        ...
        logger.warning(
            "No charset passed with PayPalIPN: %s. Guessing %s", ipn_obj.id, encoding
        )
